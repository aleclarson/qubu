...
test("reads MySQL version gates, native columns, defaults, generated columns, and table metadata", async () => {
  ...
  expect(columns[3]).toMatchObject({
    ...
    default: {
      kind: "expression",
      expression: { text: "CURRENT_TIMESTAMP" },
    },
  ...
  expect(fake.calls.filter((call) => call.parameters[0] === "shop")).toHaveLength(12)
})
...
test("normalizes complete MySQL object families, retains boundaries, and maps Snapshot v2", async () => {
  ...
  expect(fake.calls.find((call) => call.text === mysqlCollationsQuery)?.parameters).toEqual([
    "shop",
    "shop",
  ])
...
test("reports query diagnostics without leaking driver errors and preserves parameters", async () => {
  ...
  expect(calls.find((call) => call.text === mysqlTablesQuery)?.parameters).toEqual(["private"])
})
