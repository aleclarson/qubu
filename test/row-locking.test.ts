...
test('renders MySQL-compatible row locks without adding parameters', () => {
  ...
  expect(
    render(
      select({ id: users.id }, from(users), rowLock('update', 'skip-locked')),
      mysqlDialect()
    ).text
...
test('reports MySQL row-lock modes that are not supported', () => {
  ...
  try {
    ...
    expect((error as QueryValidationError).context).toBe(
      'dialect.mysql.row-lock'
    )
  }
})
...
test('reports duplicate and malformed row-lock clauses structurally', () => {
  ...
  expect(() => rowLock('invalid-mode' as never)).toThrowError(
    QueryValidationError
  )
  expect(() => rowLock('update', 'invalid-wait' as never)).toThrowError(
    QueryValidationError
  )
})
