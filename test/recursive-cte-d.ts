...
export type RecursiveDomainsPreserveAnchor = Assert<
  Equal<[SqlTypeOf<typeof recursive.id>, SqlTypeOf<typeof recursive.label>], [SqlInteger, SqlText]>
>
...
const correlatedRecursive = recursiveCte("correlated_recursive_records", correlatedAnchor, (self) =>
  select(
    {
      id: self.id,
      label: self.label,
...
export type RecursiveOuterRequirementsPropagate = Assert<
  Equal<RequiresOuterOf<typeof correlatedRecursive>, SourceIdentity<typeof records>>
>

const capableRecursive = recursiveCte("capable_recursive_records", anchor, (self) =>
  select(
    {
      id: self.id,
      label: self.label,
...
recursiveCte("missing_field", anchor, (self) => select({ id: self.id }, from(self)))
