...
function snapshot(
  ...
  dialect: SchemaSnapshot['dialect'] = { name: 'neutral', version: 1 }
): SchemaSnapshot {
  return {
    format: 'qubu-schema',
    version: 1,
...
test('orders parent creation before child creation', () => {
  const diff = diffSnapshots(
    snapshot([]),
    snapshot([table('accounts', [column('id')])])
  )
  const result = createMigrationPlan(diff)

  expect(result.ok).toBe(true)
  if (!result.ok) return
  const tableOperation = result.plan.operations.find(
    operation => operation.kind === 'table'
  )!
  const columnOperation = result.plan.operations.find(
    operation => operation.kind === 'column'
  )!
...
test('requires a decision for explicit physical renames', () => {
  const diff = diffSnapshots(
    snapshot([table('legacy_accounts')]),
...
test('keeps opaque facts blocked without inferring SQL', () => {
  const before = {
    ...
    namespace: { kind: 'mysql-database' as const, name: 'app' },
    capabilities: {
  ...
  const after = { ...before, opaqueObjects: [] }
  const diff = diffSnapshots(before, after)
  const result = createMigrationPlan(diff)

  expect(result.ok).toBe(false)
  expect(
    result.plan.operations.some(operation => operation.kind === 'opaque-object')
  ).toBe(true)
  expect(
    result.plan.operations.some(operation => operation.customSql !== undefined)
  ).toBe(false)
  expect(
    result.plan.diagnostics.some(
      diagnostic => diagnostic.code === 'decision-required'
    )
  ).toBe(true)
})
...
test('retains explicit custom SQL as tagged data', () => {
  ...
  expect(blocked.ok).toBe(false)
  const custom = blocked.plan.operations.find(
    operation => operation.type === 'custom-sql'
  )!
  expect(custom.customSql).toMatchObject(customSql)
  expect(custom.position).toBe(0)
  expect(custom.customSql?.position).toBe(7)
  expect(
    blocked.plan.operations.some(
      operation => operation.path.join('.') === 'opaque'
    )
  ).toBe(false)
})
...
test('encodes deterministically and rejects malformed plans', () => {
  ...
  expect(decodeMigrationPlan({ ...result.plan, version: 99 })).toMatchObject({
    ok: false,
  })
  expect(
    decodeMigrationPlan({
  ...
  expect(
    decodeMigrationPlan({ ...result.plan, operations: [null] })
  ).toMatchObject({ ok: false })
})
