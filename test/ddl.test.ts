...
test('emits deterministic table and column SQL for all first-party dialects', () => {
  const postgres = emitPostgresMigrationPlan(
    planFor({ name: 'postgresql', version: 1 })
  )
  const sqlite = emitSqliteMigrationPlan(
    planFor({ name: 'sqlite', version: 1 })
  )
  const mysql = emitMysqlMigrationPlan(planFor({ name: 'mysql', version: 1 }))
  ...
  expect(sqlite.statements[0]?.sql).toBe(
    'CREATE TABLE "main"."accounts" ("name" TEXT NOT NULL)'
  )
...
test('emits child constraints and indexes when a table is created', () => {
  const dialect = { name: 'postgresql', version: 1 } as const
  const accounts = {
    ...
    indexes: [
      {
        ...
        terms: [{ kind: 'column' as const, column: 'name' }],
        unique: false,
        candidateKey: false,
      },
    ],
...
test('rejects SQLite table constraints without a rebuild path', () => {
  ...
  expect(emission.diagnostics.some(item => item.code === 'unsupported')).toBe(
    true
  )
})
...
test('rejects blocked rename plans before rendering', () => {
  ...
  expect(emission.ok).toBe(false)
  expect(emission.statements).toEqual([])
  expect(emission.diagnostics.some(item => item.code === 'blocked-plan')).toBe(
    true
  )
  ...
  if (!reviewedPlan.ok) throw new Error('Expected reviewed rename plan')
...
test('reports version and transaction conflicts before SQL', () => {
  ...
  if (!planned.ok) throw new Error('Expected destructive plan to be allowed')
  ...
  expect(
    result.diagnostics.some(item => item.code === 'transaction-conflict')
  ).toBe(true)
})
...
test('reports malformed plans and unsupported dialect capabilities before SQL', () => {
  ...
  const dialect = { name: 'mysql', version: 1 } as const
