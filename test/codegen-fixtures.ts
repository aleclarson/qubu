...
const sql = (text: string): CatalogSqlExpression => ({
  ...
  dialect,
  text,
  provenance: { kind: 'catalog', dialect },
})
...
const accountEntries = table(
  ...
  [
    ...
    },
    {
      ...
      columns: ['parent_id'],
      target: { table: 'parent_records', columns: ['id'] },
      onUpdate: 'cascade',
      onDelete: 'set-null',
      match: 'full',
      deferrable: true,
      initially: 'deferred',
      dialect: { dialect, version: 1, data: { notValid: true } },
    },
    {
      kind: 'check',
  ...
  [
    {
      ...
      dialect: { dialect, version: 1, data: { method: 'btree' } },
    },
  ]
)
...
export const codegenCatalog: IntrospectionCatalog = {
  ...
  namespace: { kind: 'postgres-schema', name: 'app_data' },
...
const alphaRecords = table(
  ...
  [
    ...
    },
    {
      ...
      columns: ['beta_id'],
      target: { table: 'beta_records', columns: ['id'] },
    },
  ]
)
  ...
  [
    {
      ...
      columns: ['alpha_id'],
      target: { table: 'alpha_records', columns: ['id'] },
    },
  ],
  [
    {
...
export const cycleCodegenCatalog: IntrospectionCatalog = {
  ...codegenCatalog,
  namespace: { kind: 'postgres-schema', name: 'cycle_data' },
...
if (!cycleResult.ok) {
  throw new Error(
    cycleResult.diagnostics.map(issue => issue.message).join('\n')
  )
}
...
if (!sqliteResult.ok) {
  throw new Error(
    sqliteResult.diagnostics.map(issue => issue.message).join('\n')
  )
}
...
export const emptyCodegenCatalog: IntrospectionCatalog = {
  ...codegenCatalog,
  namespace: { kind: 'postgres-schema', name: 'empty_data' },
  tables: [],
  deferredObjects: [],
  diagnostics: [],
}
...
if (!emptyResult.ok) {
  throw new Error(
    emptyResult.diagnostics.map(issue => issue.message).join('\n')
  )
}
