...
const rendered = render(query);`,
  ...
  const executable = result?.code.replace(`${importLine}\n`, "") + "\nreturn rendered"
...
select({ id: users.id }, from(users));`,
  ...
  expect(result?.code).not.toContain(`import { select, table, integer, from } from "qubu";`)
})
...
test("supports filters and ignores non-script modules", () => {
  ...
  expect(plugin.transform(source, "/workspace/node_modules/example.ts")).toBeNull()
})
