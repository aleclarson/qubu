...
test('renders SQLite JSON paths with scalar type guards', () => {
  ...
  expect(rendered.text).toContain(
    `(json_type("events"."payload", '$."user"."email"') IS NOT NULL)`
  )
  expect(rendered.parameters).toEqual([])
})
...
test('preserves JSON null, missing, type, and existence semantics in SQLite', () => {
  ...
  try {
    ...
    insert.run(
      JSON.stringify({
        user: { name: 'Ada', active: true, email: null },
        scores: [9.5],
      })
    )
    insert.run(
      JSON.stringify({
        user: { name: 42, active: 'yes' },
        scores: ['high'],
      })
    )
    ...
    expect(database.prepare(rendered.text).all()).toEqual([
      { name: 'Ada', score: 9.5, active: 1, hasEmail: 1 },
      { name: null, score: null, active: null, hasEmail: 0 },
      { name: null, score: null, active: null, hasEmail: 0 },
    ])
  } finally {
    database.close()
...
test('escapes path keys as JSON and SQL literals', () => {
  ...
  expect(render(escaped).text).toContain(
    `'$."owner''s"."a.b"."say\\"hi" ? (@.type() == "string")'`
  )
})

test('rejects invalid array indexes', () => {
  expect(() => jsonPath(-1)).toThrow(
    'JSON path indexes must be non-negative safe integers'
  )
  expect(() => jsonPath(1.5)).toThrow(
    'JSON path indexes must be non-negative safe integers'
  )
})
...
test('diagnoses a missing JSON renderer at runtime when types are bypassed', () => {
  const dialect = {
    ...createDialect({ name: 'claims-json', placeholder: () => '?' }),
