  ...
  account => ({
    ...
    indexes: {
      activeEmail: index([account.email], {
        ...
        dialect: { dialect: 'sqlite', auto: true },
      }),
    },
  })
)
  ...
  membership => ({
    constraints: {
      accountForeign: foreignKey(
        ...
        { onDelete: 'cascade', match: 'simple', deferrable: true }
      ),
    },
    indexes: {},
  })
...
const appSchema = schema({ memberships, accounts }, { namespace: 'main' })
...
test('serializes SQLite affinity, literals, generated columns, identities, and partial indexes', () => {
  ...
  expect(snapshot.dialect).toEqual({ name: 'sqlite', version: 1 })
...
test('keeps SQLite canonical bytes independent of registry and metadata order', () => {
  const reordered = schema({ accounts, memberships }, { namespace: 'main' })
...
test('records SQLite declared-type affinity while preserving exact native text', () => {
  ...
  expect(
    createSqliteSchemaSnapshot(schema({ custom })).tables[0]?.columns[0]
  ).toMatchObject({
    storage: {
      kind: 'native',
...
test('keeps query and snapshot dialect identities separate', () => {
  ...
  expect(
    createSqliteSchemaSnapshot(schema({ raw })).tables[0]?.columns[0]
  ).toMatchObject({
    default: {
      kind: 'expression',
      expression: {
  ...
  if (!result.ok) {
    ...
    expect(
      result.diagnostics.some(
        issue =>
          issue.code === 'unsupported-dialect-option' &&
          issue.path.includes('includedColumns')
      )
    ).toBe(true)
    expect(
      result.diagnostics.some(
