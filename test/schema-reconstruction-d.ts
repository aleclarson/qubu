...
const unknownTargets = table(
  ...
  { id: column<number>(), otherId: column<number>() },
  target => ({
...
table(
  'reconstruction_unknown_references',
...
const knownTargets = table(
  'reconstruction_known_targets',
  { id: integer() },
  target => ({
    constraints: { targetPrimary: primaryKey(target.id) },
    indexes: {},
...
expectTypeOf<
  ExpressionSqlType<typeof opaqueCheck.expression>
>().toEqualTypeOf<SqlBoolean>()
expectTypeOf<
  ExpressionOutput<typeof opaqueCheck.expression>
