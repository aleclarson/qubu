...
test('attaches deterministic portable storage to every built-in helper', () => {
  ...
  expect(definitions.text.storage).toEqual({ kind: 'portable', type: 'text' })
  expect(definitions.boolean.storage).toEqual({
    kind: 'portable',
    type: 'boolean',
  })
  expect(definitions.date.storage).toEqual({ kind: 'portable', type: 'date' })
  ...
  expect(definitions.uuid.storage).toEqual({ kind: 'portable', type: 'uuid' })
  expect(definitions.json.storage).toEqual({ kind: 'portable', type: 'json' })
...
test('does not alter application or write metadata when storage is added', () => {
  const records = table('storage_mutations', {
    ...
    label: text({ nullable: true, hasDefault: true }),
  })
