import { start, type PgTmp } from "@pg-nano/pg-tmp"
...
test("runs the shared conformance probe against a pinned pg client", async () => {
  const client = new Client({ connectionString: server.dsn })
...
test("applies and atomically journals a PostgreSQL migration through pg", async () => {
  ...
  try {
    ...
    await client.query(
      "TRUNCATE __qubu_migration_applied, __qubu_migration_attempts, __qubu_migration_checkpoints, __qubu_migration_reconciliations",
    )
    await client.query("UPDATE __qubu_migration_metadata SET head = NULL WHERE singleton = 1")
    ...
    const adapter = pgMigrationAdapter(client, {
      async readSnapshot(_client, expected) {
        const result = await client.query("SELECT to_regclass('accounts') AS value")
    ...
    await client.query("INSERT INTO accounts (value) VALUES ($1)", ["bound"])
...
test("runs the shared conformance probe against a reserved postgres.js connection", async () => {
  const sql = postgres(server.dsn, { max: 2 })
