...
test("registers tables by stable logical IDs without changing query identity", () => {
  ...
  expect(reverse.registry.accounts.physicalName).toBe(forward.registry.accounts.physicalName)
...
test("reports structured schema identity and naming diagnostics", () => {
  ...
  expect(() =>
    schema(
      {
        accounts,
        memberships: table("memberships", { id: integer() }),
      },
      {
        namingPolicy: {
          version: 1,
...
test("retains foreign keys, checks, and portable indexes as named metadata", () => {
  ...
  const memberships = table(
    ...
    (memberships) => ({
      constraints: {
        accountForeign: foreignKey([memberships.tenantId, memberships.accountId], () =>
  ...
  const profiles = table("profiles", { handle: text() }, (profiles) => ({
    constraints: {
      handleForeign: foreignKey([profiles.handle], references(handles, handles.slug)),
    },
    indexes: {},
  }))
  ...
  expect(employees.constraints.managerForeign.target).toEqual(references(employees, employees.id))
  expect(profiles.constraints.handleForeign.target).toEqual(references(handles, handles.slug))
