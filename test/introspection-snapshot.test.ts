...
const server = {
  product: 'PostgreSQL',
  rawVersion: '16.0',
  parsedVersion: { major: 16, minor: 0 },
  capabilities: {
...
test('maps falsy defaults and native storage without dropping values', () => {
  const result = mapCatalogToSnapshot(
    catalog([
      ...
      column('created', 'created', 5, {
        ...
        default: { kind: 'expression', expression: sql('CURRENT_TIMESTAMP') },
      }),
    ]),
    { namespace: 'public' }
  )
  ...
  if (!result.ok) return
  expect(result.snapshot.dialect).toEqual({ name: 'postgresql', version: 1 })
  expect(result.snapshot.tables[0]?.columns).toEqual([
    expect.objectContaining({
      id: 'count',
      storage: { kind: 'native', dialect: 'postgresql', type: 'integer' },
      default: { kind: 'literal', value: { kind: 'number', value: '0' } },
    }),
    expect.objectContaining({
      id: 'created',
      storage: {
    ...
    expect.objectContaining({
      id: 'empty',
      default: { kind: 'literal', value: { kind: 'string', value: '' } },
    }),
    expect.objectContaining({
      id: 'flag',
      default: { kind: 'literal', value: { kind: 'boolean', value: false } },
    }),
    expect.objectContaining({
      id: 'missing',
      default: { kind: 'literal', value: { kind: 'null' } },
    }),
  ])
})
...
test('carries table and column identities from a previous snapshot', () => {
  ...
  const current: IntrospectionCatalog = {
    ...baseCurrent,
    tables: [{ ...baseCurrent.tables[0]!, id: 'new-table-id' }],
  }
...
test('maps constraints and ordered index terms to Snapshot v1', () => {
  ...
  const result = mapCatalogToSnapshot(
    catalog(
      [
        column('id', 'id', 1),
        column('email', 'email', 2, { storage: { nativeType: 'text' } }),
      ],
      constraints,
      indexes
    ),
  ...
  expect(result.snapshot.tables[0]?.indexes).toEqual([
    expect.objectContaining({
      ...
      terms: [
        {
          kind: 'order',
          expression: { kind: 'column', column: 'email' },
          direction: 'DESC',
        },
      ],
    }),
...
test('rejects unresolved foreign-key references in strict mode', () => {
  ...
  const result = mapCatalogToSnapshot(
    catalog([column('owner_id', 'owner_id', 1)], [constraint]),
    { namespace: 'public' }
  )
