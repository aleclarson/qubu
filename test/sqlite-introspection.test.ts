  ...
  return {
    ...
    async query<TRow extends Row = Row>(statement: CatalogQuery) {
      return database
        ...
        .all(
          ...(statement.parameters as SQLInputValue[])
  ...
  return connection(statement => {
    ...
    if (statement.text === sqliteDatabaseListQuery)
      return [
        ...
        { seq: 1, name: 'archive', file: '/tmp/archive.sqlite' },
      ]
    ...
    if (statement.text === sqliteTableListQuery)
      return [
        ...
        { schema: 'main', name: 'child_view', type: 'view', wr: 0, strict: 0 },
        { schema: 'main', name: 'search', type: 'virtual', wr: 0, strict: 0 },
        {
          schema: 'main',
          name: 'search_content',
          type: 'shadow',
    ...
    if (
      statement.text === sqliteTableInfoQuery &&
      statement.parameters[0] === 'child_view'
    )
      return [
        {
          cid: 0,
    ...
    if (
      statement.text === sqliteForeignKeyQuery &&
      statement.parameters[0] === 'child'
    )
      return [
        {
...
test('maps complete SQLite objects without changing Snapshot v1 tables', async () => {
  ...
  if (complete.ok) {
    expect(complete.snapshot.tables.map(table => table.id)).toEqual([
      'child',
      'parent',
    ])
    expect(complete.snapshot.views.map(view => view.id)).toEqual(['child_view'])
    expect(complete.snapshot.triggers.map(trigger => trigger.id)).toEqual([
      'child_trigger',
    ])
    ...
    expect(complete.snapshot.opaqueObjects).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ objectKind: 'attached-database' }),
      ])
    )
  }
  ...
  if (v1.ok)
    expect(v1.snapshot.tables.map(table => table.physicalName)).toEqual([
      'child',
      'parent',
    ])
})
...
test('verifies SQLite catalog SQL against an in-memory database', async () => {
  ...
  try {
    ...
    const parent = catalog.tables.find(
      table => table.physicalName === 'parent'
    )!
    const child = catalog.tables.find(table => table.physicalName === 'child')!
...
test('defers unrecoverable SQLite definitions with diagnostics', async () => {
  const fake = connection(statement => {
    if (statement.text === sqliteServerQuery) return [{ version: '3.45.1' }]
    if (statement.text === sqliteTableListQuery)
      return [{ schema: 'main', name: 'broken_view', type: 'view' }]
    if (statement.text === sqliteSchemaQuery)
      return [{ type: 'view', name: 'broken_view', tbl_name: 'broken_view' }]
    return []
  })
  const catalog = await readSqliteCatalog(fake.connection, options())
