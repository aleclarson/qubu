...
test('serializes the complete neutral table model without executable values', () => {
  ...
  expect(snapshot).toMatchObject({
    ...
    namingPolicy: { name: 'snake-case', version: 1 },
    namespace: 'public',
  })
  expect(snapshot.tables.map(table => table.id)).toEqual([
    'accounts',
    'memberships',
  ])
  expect(snapshot.tables[0]?.columns.map(column => column.id)).toEqual([
    'display',
    'email',
    'id',
  ])
  ...
  expect(
    snapshot.tables[0]?.constraints.map(constraint => constraint.id)
  ).toEqual(['accountPrimary', 'displayCheck', 'emailConstraint'])
...
test('retains custom schema naming policy identity beside materialized names', () => {
  const custom = schema(
    { accounts },
    { namingPolicy: { version: 1, tableName: id => `tenant_${id}` } }
  )
  const snapshot = createSchemaSnapshot(custom)

  expect(snapshot.namingPolicy).toEqual({ name: 'custom', version: 1 })
...
test('reports broken foreign-key references as structured diagnostics', () => {
  ...
  const malformed = JSON.parse(encoded) as {
    tables: Array<{
      constraints: Array<{ kind: string; target?: { table: string } }>
    }>
  }
  ...
  if (!result.ok)
    expect(
      result.diagnostics.some(issue => issue.code === 'invalid-cross-reference')
    ).toBe(true)
})
