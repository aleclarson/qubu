...
test("renders deterministic scalar expressions with schema literals", () => {
  ...
  expect(renderSchemaSql(value("O'Reilly"), { mode: "default" })).toBe("'O''Reilly'")
})

test("uses bare physical columns outside default expressions", () => {
  expect(renderSchemaExpression(gt(users.age, 18), "generated").text).toBe('("age" > 18)')
  expect(() => renderSchemaExpression(users.age, { mode: "default" })).toThrowError(
    /cannot reference table columns/,
  )
})

test("rejects parameters, query-only expressions, and unbranded extensions", () => {
  const parameterized = makeExpression<ResultMeta<number>, "function">("function", (context) =>
    context.parameter(1),
  )
  ...
  expect(() => renderSchemaExpression(schemaExpression(count()), { mode: "index" })).toThrowError(
    /Aggregates, windows, and subqueries/,
  )
})
...
test("supports explicit deterministic extensions and dialect literals", () => {
  ...
  expect(renderSchemaExpression(extension, { mode: "default" }).text).toBe("CURRENT_DATE")
  expect(renderSchemaExpression(gt(users.age, 18), "check", dialect).text).toBe('("age" > NUM(18))')
})
...
test("exposes a nominal schema-expression type", () => {
  ...
  expectTypeOf<SchemaExpressionMode>().toEqualTypeOf<"default" | "generated" | "check" | "index">()
})
