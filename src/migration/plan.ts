...
const lockValues = new Set<MigrationLockRequirement>([
  'none',
  'shared',
  'exclusive',
  'unknown',
...
export function createMigrationPlan(
  ...
  for (const context of contexts) {
    ...
    context.operation = updateOperation(context.operation, { status, decision })
  }
  ...
  const plan = freezePlan({
    ...
    ...(diff.beforeDigest === undefined
      ? {}
      : { beforeDigest: diff.beforeDigest }),
    ...(diff.afterDigest === undefined
      ? {}
      : { afterDigest: diff.afterDigest }),
    safety,
    ready,
    operations,
    dependencies,
  ...
  return ready
    ? { ok: true, plan, diagnostics: planDiagnostics }
    : { ok: false, plan, diagnostics: planDiagnostics }
}
...
export function migrationPlanDigest(input: MigrationPlan | string): string {
  const plan =
    typeof input === 'string'
      ...
      : { ok: true as const, value: assertMigrationPlan(input) }
...
function preconditionsFor(
  source: SnapshotDiffOperation
): readonly MigrationPrecondition[] {
  ...
  const result: MigrationPrecondition[] = [
    {
      ...
      ...(source.before === undefined
        ? {}
        : { digest: digestJson(source.before.value) }),
    },
  ]
...
function classifySafety(
  ...
  const relevant = diagnostics.filter(diagnostic =>
    pathRelated(diagnostic.path, source.path)
  )
  if (
    relevant.some(
      diagnostic => diagnostic.code === 'unknown' || diagnostic.code === 'lossy'
    ) ||
  ...
  if (
    source.destructive ||
    relevant.some(diagnostic => diagnostic.code === 'destructive')
  )
    return 'destructive'
  if (
...
function addCustomSql(
  ...
  for (const [index, input] of inputs.entries()) {
    if (!isRecord(input)) {
      diagnostics.push(
        planDiagnostic('custom-sql', 'Custom SQL input must be an object', [
          'customSql',
          index,
        ])
      )
    ...
    if (
      ...
      diagnostics.push(
        planDiagnostic(
          'custom-sql',
          'Custom SQL needs a safety classification and reason',
          ['customSql', index]
        )
      )
      continue
    ...
    if (
      ...
      !validPosition(candidate.position)
    ) {
      diagnostics.push(
        planDiagnostic(
          'custom-sql',
          'Custom SQL position must be a non-negative integer',
          ['customSql', index, 'position']
        )
      )
      continue
    }
    if (
      candidate.reversible !== undefined &&
    ...
    if (
      ...
      diagnostics.push(
        planDiagnostic(
          'custom-sql',
          'Custom SQL dependsOn must contain operation IDs',
          ['customSql', index, 'dependsOn']
        )
      )
      continue
    }
    ...
    if (candidate.kind !== undefined && !isObjectKind(candidate.kind)) {
      diagnostics.push(
        planDiagnostic('custom-sql', 'Custom SQL kind is invalid', [
          'customSql',
          index,
          'kind',
        ])
    ...
    if (
      ...
      diagnostics.push(
        planDiagnostic(
          'custom-sql',
          'Custom SQL operationId must be a string',
    ...
    if (operationIds.has(id)) {
      diagnostics.push(
        planDiagnostic(
          'custom-sql',
          `Duplicate custom SQL operation ID "${id}"`,
          ['customSql', index]
        )
      )
      continue
    ...
    operationIds.add(id)
    const targetObject =
      target?.operation.origin?.after ?? target?.operation.origin?.before
    ...
    const operation: MigrationOperation = {
      ...
      ...(targetObject?.provenance === undefined
        ? {}
        : { provenance: targetObject.provenance }),
...
function resolveCustomTarget(
  ...
  if (input.operationId !== undefined)
    for (const target of targets.values())
      if (target.operation.id === input.operationId) return target
...
function findDecision(
  ...
  const direct = decisions.find(decision => {
    ...
    if (
      ...
      (decision.namespace === undefined ||
        decision.namespace === context.operation.namespace) &&
      (decision.path === undefined ||
        samePath(decision.path, context.operation.path))
    )
      return true
    return false
  })
  ...
  if (relevantCode !== undefined)
    return decisions.find(
      decision => decision.code === mapDiffCode(relevantCode)
    )
  if (optionAllows(context.operation.safety, options)) return undefined
  return undefined
}
...
function buildDependencies(
  ...
  for (const context of contexts) {
    const object =
      context.operation.origin?.after ?? context.operation.origin?.before
    if (object !== undefined) byObject.set(targetKeyFromObject(object), context)
  }
  const addEdge = (
  ...
  for (const context of contexts) {
    ...
    if (parent !== undefined && object !== undefined) {
      const parentContext = byObject.get(
        targetKeyFromReference(parent, object.namespace)
      )
    ...
    if (operation.type !== 'remove' && object !== undefined)
      for (const reference of referencedObjects(object.value)) {
        const referenceContext = byObject.get(
          targetKeyFromReference(reference, object.namespace)
        )
        if (referenceContext !== undefined)
          addEdge(
            referenceContext.operation,
            operation,
            'reference-before-dependent'
          )
      }
...
function topologicalOrder(
  ...
  if (result.length !== contexts.length) {
    diagnostics.push(
      planDiagnostic(
        'dependency-cycle',
        'Migration operation dependencies contain a cycle',
        []
      )
    )
...
function referencedObjects(
  value: Readonly<Record<string, SnapshotJsonValue>>
): readonly { readonly kind: SnapshotDiffObjectKind; readonly id: string }[] {
  const result: { kind: SnapshotDiffObjectKind; id: string }[] = []
  const visit = (current: SnapshotJsonValue, key: string | undefined): void => {
    ...
    if (
      ...
      result.push({ kind: current.kind, id: current.id })
...
function operationId(source: SnapshotDiffOperation): string {
  ...
  if (source.namespace !== undefined) identity.namespace = source.namespace
  if (source.logicalId !== undefined) identity.logicalId = source.logicalId
  if (source.physicalName !== undefined)
    identity.physicalName = source.physicalName
  return `op_${digestText(canonicalJson(toSnapshotJsonValue(identity))).slice(
    'fnv1a64:'.length
  )}`
}

function customOperationId(
  input: MigrationCustomSqlInput,
  index: number
): string {
  const identity: Record<string, unknown> = {
  ...
  if (input.reversible !== undefined) identity.reversible = input.reversible
  if (input.operationId !== undefined) identity.operationId = input.operationId
  if (input.kind !== undefined) identity.kind = input.kind
  if (input.namespace !== undefined) identity.namespace = input.namespace
  if (input.path !== undefined) identity.path = input.path
  if (input.position !== undefined) identity.position = input.position
  if (input.dependsOn !== undefined) identity.dependsOn = input.dependsOn
  return `custom_${digestText(
    canonicalJson(toSnapshotJsonValue(identity))
...
function targetKeyFromObject(object: SnapshotDiffObject): string {
  return targetKeyFromReference(
    { kind: object.kind, id: object.id },
    object.namespace
  )
}

function targetKeyFromReference(
  reference: { readonly kind: SnapshotDiffObjectKind; readonly id: string },
  ...
  for (const [index, value] of values.entries()) {
    if (
      ...
      diagnostics.push(
        planDiagnostic(
          'invalid-plan',
          'Migration decisions need action and a non-empty reason',
          ['decisions', index]
        )
      )
      continue
...
function sortDecisions(
  values: readonly MigrationDecision[]
): readonly MigrationDecision[] {
  return freeze(
    [...values].sort(
      (left, right) =>
...
function mapDiffDiagnostic(
  source: SnapshotDiffDiagnostic
): MigrationDiagnostic {
...
function pathStarts(path: SnapshotDiffPath, prefix: SnapshotDiffPath): boolean {
  return (
    ...
    prefix.every((item, index) => item === path[index])
  )
}
...
function sortDiagnostics(
  values: readonly MigrationDiagnostic[]
...
function validatePlanValue(input: unknown):
  | { readonly ok: true; readonly value: MigrationPlan }
  | {
      readonly ok: false
  ...
  if (!isRecord(value))
    return {
      ok: false,
      diagnostics: freeze([
        planDiagnostic('invalid-plan', 'Migration plan must be an object', []),
      ]),
    }
  try {
  ...
  if (plan.format !== migrationPlanFormat)
    diagnostics.push(
      planDiagnostic(
        'invalid-plan',
        `Migration plan format must be "${migrationPlanFormat}"`,
        ['format']
      )
    )
  ...
  if (!isSafety(plan.safety))
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Plan safety is invalid', ['safety'])
    )
  if (typeof plan.ready !== 'boolean')
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Plan ready must be boolean', ['ready'])
    )
  if (!Array.isArray(plan.operations))
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Plan operations must be an array', [
        'operations',
      ])
    )
  if (!Array.isArray(plan.dependencies))
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Plan dependencies must be an array', [
        'dependencies',
      ])
    )
  if (!Array.isArray(plan.decisions))
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Plan decisions must be an array', [
        'decisions',
      ])
    )
  if (!Array.isArray(plan.diagnostics))
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Plan diagnostics must be an array', [
        'diagnostics',
      ])
    )
  if (
    typeof plan.beforeDigest === 'string' ||
    plan.beforeDigest === undefined
  ) {
    // Optional digest has no further shape requirements.
  } else
    diagnostics.push(
      planDiagnostic('invalid-plan', 'beforeDigest must be a string', [
        'beforeDigest',
      ])
    )
  if (typeof plan.afterDigest === 'string' || plan.afterDigest === undefined) {
    ...
    diagnostics.push(
      planDiagnostic('invalid-plan', 'afterDigest must be a string', [
        'afterDigest',
      ])
    )
  if (Array.isArray(plan.operations))
  ...
  if (
    ...
    const expectedReady =
      ...
      plan.operations.every(
        operation => operation.status !== 'decision-required'
      )
    ...
    if (isSafety(plan.safety)) {
      ...
      if (plan.safety !== expectedSafety)
        diagnostics.push(
          planDiagnostic(
            'non-canonical',
            'Plan safety must match its highest operation safety',
            ['safety']
          )
        )
    }
  ...
  if (diagnostics.length > 0)
    return { ok: false, diagnostics: freeze(sortDiagnostics(diagnostics)) }
  return { ok: true, value: freezePlan(plan) }
}
...
function validateOperations(
  ...
  for (const [index, operation] of operations.entries()) {
    ...
    if (!isRecord(operation)) {
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Operation must be an object', path)
      )
      continue
    }
    ...
    if (typeof candidate.id !== 'string' || candidate.id.length === 0)
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Operation ID must be non-empty', [
          ...path,
          'id',
        ])
      )
      ...
      diagnostics.push(
        planDiagnostic(
          'invalid-plan',
          `Duplicate operation ID "${candidate.id}"`,
          [...path, 'id']
        )
      )
    ...
    if (!operationValues.has(candidate.type))
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Operation type is invalid', [
          ...path,
          'type',
        ])
      )
    if (!isObjectKind(candidate.kind) && candidate.kind !== 'custom-sql')
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Operation kind is invalid', [
          ...path,
          'kind',
        ])
      )
    ...
    if (candidate.type === 'custom-sql' && candidate.customSql === undefined)
      diagnostics.push(
        planDiagnostic(
          'custom-sql',
          'Custom SQL operation is missing its explicit SQL record',
          [...path, 'customSql']
        )
      )
    if (candidate.type === 'custom-sql' && candidate.customSql !== undefined)
    ...
    if (candidate.type !== 'custom-sql' && candidate.customSql !== undefined)
      diagnostics.push(
        planDiagnostic(
          'invalid-plan',
          'Only custom-sql operations may contain customSql',
          [...path, 'customSql']
        )
      )
    if (!isSafety(candidate.safety))
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Operation safety is invalid', [
          ...path,
          'safety',
        ])
      )
    if (!lockValues.has(candidate.lock))
      diagnostics.push(
        planDiagnostic(
          'invalid-plan',
          'Operation lock requirement is invalid',
          [...path, 'lock']
        )
      )
    if (!transactionValues.has(candidate.transaction))
      diagnostics.push(
        planDiagnostic(
          'invalid-plan',
          'Operation transaction requirement is invalid',
          [...path, 'transaction']
        )
      )
    if (!isPath(candidate.path))
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Operation path must be an array', [
          ...path,
          'path',
        ])
      )
    if (!isDialect(candidate.dialect))
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Operation dialect is invalid', [
          ...path,
          'dialect',
        ])
      )
    ...
    if (
      ...
      diagnostics.push(
        planDiagnostic(
          'invalid-plan',
          'Operation reversibility does not match reversible',
          [...path, 'reversibility']
        )
      )
    ...
    if (!Array.isArray(candidate.preconditions))
      diagnostics.push(
        planDiagnostic(
          'invalid-plan',
          'Operation preconditions must be an array',
          [...path, 'preconditions']
        )
      )
    else
    ...
    if (!Array.isArray(candidate.dependsOn))
      ...
      if (
        ...
        diagnostics.push(
          planDiagnostic(
            'invalid-plan',
            'Operation dependsOn must contain operation IDs',
            [...path, 'dependsOn']
          )
        )
      if (!isSorted(candidate.dependsOn))
        diagnostics.push(
          planDiagnostic(
            'non-canonical',
            'Operation dependsOn must be sorted',
            [...path, 'dependsOn']
          )
        )
    }
    ...
    if (
      ...
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Operation status is invalid', [
          ...path,
          'status',
        ])
      )
    ...
    if (candidate.position !== index)
      diagnostics.push(
        planDiagnostic(
          'non-canonical',
          'Operations must use contiguous topological positions',
          [...path, 'position']
        )
      )
    ...
    if (
      ...
      typeof candidate.irreversibleReason !== 'string'
    )
      diagnostics.push(
        planDiagnostic(
          'invalid-plan',
          'Irreversible operations need a reason',
          [...path, 'irreversibleReason']
        )
      )
  }
}
...
function validateDependencies(
  ...
  const positions = new Map(
    validOperations.map((operation, index) => [operation.id, index])
  )
  ...
  for (const [index, dependency] of plan.dependencies.entries()) {
    ...
    if (!isRecord(dependency)) {
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Dependency must be an object', path)
      )
      continue
    }
    ...
    if (typeof candidate.from !== 'string' || typeof candidate.to !== 'string')
      diagnostics.push(
        planDiagnostic(
          'invalid-plan',
          'Dependency endpoints must be operation IDs',
          path
        )
      )
    if (!dependencyReasons.has(candidate.reason))
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Dependency reason is invalid', [
          ...path,
          'reason',
        ])
      )
    ...
    if (seen.has(key))
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Duplicate dependency edge', path)
      )
    seen.add(key)
    if (index > 0) {
      ...
      if (
        isRecord(previous) &&
        compareDependency(
          previous as unknown as MigrationDependency,
          candidate
        ) > 0
      )
        diagnostics.push(
          planDiagnostic('non-canonical', 'Dependencies must be sorted', path)
        )
    }
  }
    ...
    for (const dependency of operation.dependsOn) {
      if (
        !plan.dependencies.some(
          edge => edge.from === dependency && edge.to === operation.id
        )
      )
        diagnostics.push(
          planDiagnostic(
            'invalid-plan',
            'Operation dependsOn is missing its dependency edge',
      ...
      if (!ids.has(dependency))
        diagnostics.push(
          planDiagnostic(
            'invalid-plan',
            'Operation dependsOn references an unknown operation',
            ['operations', index, 'dependsOn']
          )
        )
    }
...
function validatePreconditions(
  ...
  for (const [index, precondition] of preconditions.entries()) {
    ...
    if (
      ...
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Precondition type is invalid', [
          ...itemPath,
          'type',
        ])
      )
    if (!isPath(precondition.path))
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Precondition path must be an array', [
          ...itemPath,
          'path',
        ])
      )
    if (!isObjectKind(precondition.kind) && precondition.kind !== 'custom-sql')
      diagnostics.push(
        planDiagnostic('invalid-plan', 'Precondition kind is invalid', [
          ...itemPath,
          'kind',
        ])
      )
    if (precondition.property !== undefined && !isPath(precondition.property))
      diagnostics.push(
        planDiagnostic(
          'invalid-plan',
          'Precondition property must be an array',
          [...itemPath, 'property']
        )
      )
    if (
      precondition.digest !== undefined &&
      typeof precondition.digest !== 'string'
    )
      diagnostics.push(
...
function validateCustomSql(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      planDiagnostic('custom-sql', 'Custom SQL record must be an object', path)
    )
    return
  }
  requireKeys(
  ...
  if (!isDialect(value.dialect))
    diagnostics.push(
      planDiagnostic('custom-sql', 'Custom SQL dialect is invalid', [
        ...path,
        'dialect',
      ])
    )
  if (!isSafety(value.safety))
    diagnostics.push(
      planDiagnostic('custom-sql', 'Custom SQL safety is invalid', [
        ...path,
        'safety',
      ])
    )
  ...
  if (typeof value.reason !== 'string' || value.reason.length === 0)
    diagnostics.push(
      planDiagnostic('custom-sql', 'Custom SQL reason must be non-empty', [
        ...path,
        'reason',
      ])
    )
...
function validateOrigin(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Operation origin must be an object', path)
    )
    return
  }
  ...
  if (
    !operationValues.has(value.type as MigrationOperationType) ||
    value.type === 'custom-sql'
  )
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Operation origin type is invalid', [
        ...path,
        'type',
      ])
    )
  if (!isObjectKind(value.kind))
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Operation origin kind is invalid', [
        ...path,
        'kind',
      ])
    )
  if (!isPath(value.path))
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Operation origin path must be an array', [
        ...path,
        'path',
      ])
    )
  if (!Array.isArray(value.evidence))
    diagnostics.push(
      planDiagnostic(
        'invalid-plan',
        'Operation origin evidence must be an array',
        [...path, 'evidence']
      )
    )
}
...
function validateDecision(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Decision must be an object', path)
    )
    return
  }
  ...
  if (value.action !== 'allow' && value.action !== 'skip')
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Decision action is invalid', [
        ...path,
        'action',
      ])
    )
  if (typeof value.reason !== 'string' || value.reason.length === 0)
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Decision reason must be non-empty', [
        ...path,
        'reason',
      ])
    )
  if (value.kind !== undefined && !isObjectKind(value.kind))
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Decision kind is invalid', [
        ...path,
        'kind',
      ])
    )
  if (value.path !== undefined && !isPath(value.path))
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Decision path must be an array', [
        ...path,
        'path',
      ])
    )
  if (value.code !== undefined && !isDiagnosticCode(value.code))
    diagnostics.push(
      planDiagnostic('invalid-plan', 'Decision code is invalid', [
        ...path,
        'code',
      ])
    )
...
function compareDependency(
  left: MigrationDependency,
  right: MigrationDependency
): number {
  return (
    left.from.localeCompare(right.from) ||
    left.to.localeCompare(right.to) ||
...
function validateDiagnostics(
  ...
  for (const [index, diagnostic] of diagnostics.entries()) {
    ...
    if (!isRecord(diagnostic)) {
      output.push(
        planDiagnostic('invalid-plan', 'Diagnostic must be an object', path)
      )
      continue
    }
    requireKeys(
    ...
    if (!isDiagnosticCode(diagnostic.code))
      output.push(
        planDiagnostic('invalid-plan', 'Diagnostic code is invalid', [
          ...path,
          'code',
        ])
      )
    if (diagnostic.severity !== 'error' && diagnostic.severity !== 'warning')
      output.push(
        planDiagnostic('invalid-plan', 'Diagnostic severity is invalid', [
          ...path,
          'severity',
        ])
      )
    if (typeof diagnostic.message !== 'string')
      output.push(
        planDiagnostic('invalid-plan', 'Diagnostic message must be a string', [
          ...path,
          'message',
        ])
      )
    if (!isPath(diagnostic.path))
      output.push(
        planDiagnostic('invalid-plan', 'Diagnostic path must be an array', [
          ...path,
          'path',
        ])
      )
...
function validateDialect(
  ...
  if (!isDialect(value)) {
    diagnostics.push(
      planDiagnostic(
        'invalid-plan',
        'Dialect must contain a name and positive version',
        path
      )
    )
  }
...
function requireKeys(
  ...
  for (const key of required)
    if (!(key in value))
      diagnostics.push(
        planDiagnostic('invalid-plan', `Missing required field "${key}"`, [
          ...path,
          key,
        ])
      )
  for (const key of Object.keys(value))
    if (!allowed.has(key))
      diagnostics.push(
        planDiagnostic('invalid-plan', `Unknown plan field "${key}"`, [
          ...path,
          key,
        ])
      )
