...
export interface UnsafeSchemaSqlExpression extends SchemaExpression<never, "unsafe"> {
...
>
export function schemaExpression(expression: AnyExpression): AnySchemaExpression {
...
>(kind: TKind, render: (context: SchemaRenderContext) => void): SchemaExpression<TMetadata, TKind> {
 ...
 */
export function unsafeSchemaSql(dialect: string, sql: string): UnsafeSchemaSqlExpression
...
export function isUnsafeSchemaSql(value: unknown): value is UnsafeSchemaSqlExpression {
  return (
    isSchemaExpression(value) &&
    value.expressionKind === "unsafe" &&
    typeof (value as Partial<UnsafeSchemaSqlExpression>).schemaSqlDialect === "string" &&
...
function renderSchemaLiteral(dialect: Dialect, value: unknown, mode: SchemaExpressionMode): string {
