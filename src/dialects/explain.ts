...
export function assertSupportedExplainOptions(
  ...
  for (const option of explainOptionNames) {
    if (options[option] === undefined || supported.includes(option)) continue
...
export const postgresExplain: DialectExplain = {
  render(statement, queryKind, options) {
    ...
    if (
      format !== undefined &&
      !postgresExplainFormats.includes(
        format as (typeof postgresExplainFormats)[number]
      )
    ) {
...
export const sqliteExplain: DialectExplain = {
  render(statement, queryKind, options) {
    ...
    if (
      format !== undefined &&
      !sqliteExplainFormats.includes(
        format as (typeof sqliteExplainFormats)[number]
      )
    ) {
    ...
    const queryPlan =
      formatIsQueryPlan || (!formatIsBytecode && options.queryPlan !== false)
...
export const mysqlExplain: DialectExplain = {
  render(statement, queryKind, options) {
    ...
    if (
      format !== undefined &&
      !mysqlExplainFormats.includes(
        format as (typeof mysqlExplainFormats)[number]
      )
    ) {
    ...
    if (format === undefined) return `EXPLAIN ${statement}`
