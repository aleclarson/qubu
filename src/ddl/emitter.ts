    ...
    emit(plan, schemaDialect, options = {}) {
      ...
      try {
        for (const operation of validated.operations) {
          ...
          if (sql === undefined || sql.length === 0) continue
          statements.push({
...
function preflight(
  ...
  try {
    ...
    if (raw?.ready === false && !options.allowBlocked && !options.allowUnsafe)
      diagnostics.push({
        ...
        severity: 'error',
        message:
          'Migration plan is blocked and needs explicit review before DDL emission',
        path: [],
      })
    if (Array.isArray(raw?.diagnostics))
      for (const item of raw.diagnostics) {
        ...
        diagnostics.push({
          ...
          operationId:
            typeof item.operationId === 'string' ? item.operationId : undefined,
        })
      }
  ...
  for (const operation of plan.operations) {
    ...
    if (operation.type === 'custom-sql') {
      if (
        operation.customSql === undefined ||
        operation.customSql.sql.trim().length === 0
      )
        diagnostics.push(
          operationDiagnostic(
            operation,
            'custom-sql',
            'Custom SQL operation has no SQL text'
          )
        )
...
function diagnoseExecutionContext(
  ...
  if (operation.lock === 'unknown')
    diagnostics.push(
      operationDiagnostic(
        operation,
        'lock-conflict',
        'Operation lock requirement is unknown'
      )
    )
  else if (
    options.lock !== undefined &&
    lockRank[operation.lock] > lockRank[options.lock]
  )
    diagnostics.push({
      ...operationDiagnostic(
        operation,
  ...
  if (operation.transaction === 'unknown')
    ...
    options.transaction === 'autocommit' &&
    ...
    options.transaction === 'managed' &&
    operation.transaction === 'forbidden'
  )
    diagnostics.push({
      ...operationDiagnostic(
        operation,
...
function diagnoseVersion(
  ...
  if (options.serverVersion === undefined) return
...
function minimumVersion(
  ...
  if (dialect === 'mysql') {
    ...
    if (operation.kind === 'constraint' && operation.type === 'add') {
      ...
      if (isRecord(value) && value.kind === 'check') return [8, 0, 16]
    }
  }
  if (
    dialect === 'postgresql' &&
    operation.kind === 'partition' &&
    operation.type === 'add'
  )
    return [10]
  return undefined
...
function diagnoseOperation(
  ...
  for (const expression of expressionsIn(value)) {
    if (
      expression.dialect !== undefined &&
      expression.dialect !== features.dialect
    )
      diagnostics.push(
        operationDiagnostic(
          operation,
  ...
  if (
    ...
    !features.supports.has('generated-column')
  )
    diagnostics.push(
      operationDiagnostic(
        operation,
  ...
  if (
    ...
    !features.supports.has('index-predicate')
  )
    diagnostics.push(
      operationDiagnostic(
        operation,
...
function isOperationSupported(
  ...
  if (
    ...
    return false
  if (
    operation.kind === 'materialized-view' &&
...
function renderPropertyChange(
  ...
  if (operation.kind === 'index') {
    ...
    const created = renderCreateIndex(
      operation,
      value,
      operations,
      dialect,
    ...
    if (dropped === undefined) return created
    if (created === undefined) return dropped
    return `${dropped};\n${created}`
  }
  ...
  if (operation.kind === 'routine') {
    ...
    const dropped =
      before === undefined
        ? undefined
        : renderDropRoutine(operation, before, dialect, features)
...
function renderCreateTable(
  ...
  const definitions = columns.map(column =>
    renderColumn(column, dialect, features)
  )
  const name = requiredName(operation, value)
...
function renderColumn(
  ...
  const parts = [
    ...
    renderStorage(value.storage, dialect),
  ]
  ...
  if (value.default !== undefined) {
    ...
    throw new TypeError(
      'Column declares a default but no default value was retained'
    )
  }
  ...
  if (identity !== undefined) {
    if (features.dialect === 'postgresql')
      ...
      throw new TypeError(
        'SQLite identity declarations are not supported by this emitter'
      )
  }
...
function renderStorage(value: unknown, dialect: SchemaDialect): string {
  ...
  if (rendered === undefined)
    throw new TypeError(
      `Unsupported portable storage type "${type}" for ${dialect.name}`
    )
  return rendered
}
...
function schemaLiteral(dialect: SchemaDialect, value: unknown): string {
  ...
  if (rendered !== undefined) {
    if (rendered.includes('?'))
      throw new TypeError(
        'Schema literal renderer returned a parameter placeholder'
      )
    return rendered
  }
...
function renderAddConstraint(
  ...
  if (kind === 'primary-key') body = `PRIMARY KEY (${terms})`
    ...
    const targetTable =
      target === undefined ? undefined : recordValue(target.table)
    const targetName =
      targetTable === undefined
        ? undefined
        : (stringValue(targetTable.id) ?? stringValue(targetTable.physicalName))
    const targetColumns =
      target === undefined ? [] : stringArray(target.columns)
    if (
  ...
  if (deferrable) {
    if (features.dialect === 'postgresql') {
      ...
      throw new TypeError(
        `${features.dialect} does not support deferrable constraints`
      )
  }
...
function renderDropConstraint(
  ...
  throw new TypeError(
    'SQLite cannot drop table constraints without rebuilding the table'
  )
}
...
function renderCreateIndex(
  ...
  if (predicate !== undefined) sql += ` WHERE ${predicate.sql}`
  return sql
}
...
function renderCreateView(
  ...
  if (
    ...
    throw new TypeError(
      `${features.dialect} does not support materialized views`
    )
...
function renderReplaceView(
  ...
  if (operation.kind === 'materialized-view') {
    if (features.dialect !== 'postgresql')
      throw new TypeError(
        `${features.dialect} cannot replace materialized views`
      )
  ...
  throw new TypeError(
    'SQLite cannot alter a view definition without an explicit rebuild'
  )
}
...
function renderCreateEnum(
  ...
  if (features.dialect !== 'postgresql')
    throw new TypeError(
      `${features.dialect} does not support standalone enum types`
    )
  const name = requiredName(operation, value)
  const values = arrayOfRecords(value.values).sort(
    (left, right) =>
      numberValue(left.ordinalPosition) - numberValue(right.ordinalPosition)
  )
  if (values.length === 0)
...
function renderCreateCollation(
  ...
  throw new TypeError(
    'MySQL collations are server metadata and cannot be created as named objects'
  )
}
...
function renderCreateTrigger(
  ...
  if (condition !== undefined) sql += ` WHEN (${condition.sql})`
  return sql
}
...
function renderCreateRoutine(
  ...
  if (kind !== 'function' && kind !== 'procedure')
    throw new TypeError(
      `Routine kind "${String(kind)}" is not supported by this emitter`
    )
  const parameters = arrayOfRecords(value.parameters)
    ...
    .join(', ')
  const body = expressionValue(value.body)
  if (body === undefined) throw new TypeError('Routine body is missing')
...
function renderRoutineParameter(
  value: JsonRecord,
  dialect: SchemaDialect
): string {
...
function renderCreatePolicy(
  ...
  if (features.dialect !== 'postgresql')
    throw new TypeError(
      `${features.dialect} does not support row-level policies`
    )
  const name = requiredName(operation, value)
  ...
  if (using !== undefined) sql += ` USING (${using.sql})`
  if (check !== undefined) sql += ` WITH CHECK (${check.sql})`
  return sql
}
...
function renderCreateExtension(
  ...
  if (features.dialect !== 'postgresql')
    throw new TypeError(
      `${features.dialect} does not support PostgreSQL extensions`
    )
  ...
  if (schema !== undefined) sql += ` SCHEMA ${dialect.quoteIdentifier(schema)}`
  return sql
}
...
function renderComment(
  ...
  const targetName =
    target === undefined
      ? undefined
      : (stringValue(target.id) ?? stringValue(target.physicalName))
  ...
  if (features.dialect === 'mysql') {
    if (name === undefined) throw new TypeError('Comment target is incomplete')
    if (targetKind === 'column') {
      if (text === null)
        throw new TypeError(
          'MySQL column comments require a full column definition'
        )
      throw new TypeError(
        'MySQL column comments require an explicit full ALTER TABLE definition'
      )
    }
...
function renderOwnership(
  ...
  if (features.dialect !== 'postgresql')
    throw new TypeError(
      `${features.dialect} does not support object ownership DDL`
    )
...
function renderAlterColumn(
  ...
  if (features.dialect === 'sqlite')
    throw new TypeError(
      'SQLite cannot alter column definitions without rebuilding the table'
    )
  if (features.dialect === 'mysql')
  ...
  if (
    !sameValue(before.default, value.default) ||
    before.hasDefault !== value.hasDefault
  ) {
    if (value.default === undefined)
      statements.push(
  ...
  if (
    before.generatedColumn !== value.generatedColumn ||
...
function parentTable(
  ...
  const parent =
    operation.origin?.after?.parent ?? operation.origin?.before?.parent
  const candidate =
    parent === undefined
      ? undefined
      : operations.find(
...
function tableReference(
  ...
  const physicalName =
    value === undefined ? undefined : stringValue(value.physicalName)
  const candidate =
    id === undefined
      ? undefined
      : operations.find(
...
function requiredName(operation: MigrationOperation, value: unknown): string {
  ...
  if (name === undefined || name.length === 0)
    throw new TypeError(
      `Operation "${operation.id}" is missing a physical name`
    )
  return name
}
...
function operationName(
  ...
  return (
    operation.physicalName ??
    stringValue(value.physicalName) ??
...
function emission(
  ...
  const sql = frozenStatements
    .map(statement =>
      statement.sql.trimEnd().endsWith(';')
        ? statement.sql.trimEnd()
        : `${statement.sql};`
    )
    .join('\n')
  return Object.freeze({
    ...
    sql,
    parameters: Object.freeze(
      frozenStatements.flatMap(statement => statement.parameters)
    ),
  })
}
...
function dollarQuote(sql: string): string {
  ...
  for (const tag of tags) if (!sql.includes(tag)) return `${tag}${sql}${tag}`
