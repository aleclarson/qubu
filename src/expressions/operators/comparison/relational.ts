...
type ComparisonResult<TLeft, R, TOperator extends string> = ResultExpression<{
  ...
  readonly nullableFrom: TOperator extends "IS DISTINCT FROM" | "IS NOT DISTINCT FROM"
    ? never
    : TOperator extends "=" | "<>"
...
export type ComparisonValidation<TLeft, TRight, TOperator extends string> = TOperator extends
  | "LIKE"
  | "NOT LIKE"
  | "ILIKE"
  ? SqlCapabilityValidation<ExpressionSqlType<TLeft>, SqlTextLike> &
      SqlCapabilityValidation<OperandSqlType<TRight, ExpressionSqlType<TLeft>>, SqlTextLike>
  : TOperator extends "<" | "<=" | ">" | ">="
...
export function eq<T, TLeft extends ExpressionWithOutput<T>, R extends Operand<NoInfer<T>>>(
  left: TLeft & ComparisonValidation<TLeft, R, "=">,
  right: R,
) {
  return comparison("=", left, right)
}

export function ne<T, TLeft extends ExpressionWithOutput<T>, R extends Operand<NoInfer<T>>>(
  left: TLeft & ComparisonValidation<TLeft, R, "<>">,
  right: R,
) {
  return comparison("<>", left, right)
...
export function like<TLeft extends ExpressionWithOutput<string>, R extends Operand<string>>(
  left: TLeft & ComparisonValidation<TLeft, R, "LIKE">,
  pattern: R,
) {
  return comparison("LIKE", left, pattern)
}

export function notLike<TLeft extends ExpressionWithOutput<string>, R extends Operand<string>>(
  left: TLeft & ComparisonValidation<TLeft, R, "NOT LIKE">,
  pattern: R,
) {
...
export function isNotDistinctFrom<
  ...
  TLeft extends ExpressionWithOutput<T>,
  R extends Operand<NoInfer<T>>,
>(left: TLeft & ComparisonValidation<TLeft, R, "IS NOT DISTINCT FROM">, right: R) {
