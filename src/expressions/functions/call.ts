...
export type FunctionArguments<T extends readonly unknown[]> = RequiresOf<T[number]>
...
export function typedCall<TSqlType extends AnySqlType, TOutput = unknown>() {
  return <const TName extends string, const TArguments extends readonly unknown[]>(
    name: TName,
    ...args: TArguments
  ) => call<TOutput, TName, TArguments, NullabilityOf<TArguments[number]>, TSqlType>(name, ...args)
}
