...
export type ResultDecoder<T = unknown> = (
  value: unknown,
  context: ResultDecodeContext
) => T

/** Driver-specific decoders selected by a field's portable result domain. */
export type ResultDecoders = Readonly<
  Partial<Record<DecodableResultType, ResultDecoder>>
>
...
export const resultValueMetadata: unique symbol = Symbol(
  'qubu.result-value-metadata'
)
...
export function resultShapeValue(
  ...
  return field === undefined
    ? undefined
    : resultValue(field.type, field.decoder)
}
...
export const dateResultDecoder: ResultDecoder<Date> = value => {
  ...
  if (
    Number.isNaN(decoded.getTime()) ||
    decoded.toISOString().slice(0, 10) !== value
  )
...
export class ResultDecodingError extends TypeError {
  ...
  constructor(
    field: string,
    rowIndex: number,
    resultType: DecodableResultType | undefined
  ) {
    super(
...
export function decodeResultRow<TRow extends object>(
  ...
  for (const field of shape.fields) {
    const decoder =
      field.decoder ?? (field.type ? decoders?.[field.type] : undefined)
    if (decoder === undefined || row[field.name] === null) continue
    try {
      decoded ??= { ...row }
