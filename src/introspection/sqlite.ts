...
export async function readSqliteCatalog(
  ...
  const tableSql = new Map(
    schemaRows
      ...
      .filter(
        (entry): entry is readonly [string, string | undefined] =>
          entry[0] !== undefined
      )
  )
  const tables = tableRows
    ...
    .map(row =>
      table(
        row,
        tableSql.get(text(row.name) ?? '') ?? undefined,
        options.namespace
      )
    )
  ...
  for (const row of databaseRows) {
    ...
    diagnostics.push(
      createIntrospectionDiagnostic({
        ...
        remediation:
          'Run a separate catalog read with this database name to inspect it.',
      })
    )
  }
  const views: CatalogView[] = []
  for (const row of tableRows.filter(
    row => normalizeType(row.type) === 'view'
  )) {
  ...
  for (const currentTable of tables) {
    ...
    const rowidPrimaryKey =
      rows.filter(row => (number(row.pk) ?? 0) > 0).length === 1
    const columns = rows
    ...
    for (const indexRow of indexRows) {
      ...
      if (
        !indexName ||
        (indexName.startsWith('sqlite_autoindex_') &&
          text(indexRow.origin)?.toLowerCase() !== 'u')
      )
        continue
  ...
  for (const view of views) {
    ...
    ;(view as Mutable<CatalogView>).columns = rows
      .filter(row => number(row.hidden) !== 1)
      .map(row =>
        column(row, view, options.namespace, undefined, false, diagnostics)
      )
  }
  for (const row of schemaRows.filter(
    row => normalizeType(row.type) === 'trigger'
  )) {
  ...
  return Object.freeze({
    ...
    namespace: {
      ...
      dialect: sqliteExtension({
        ...
        generatedSql: true,
        visibility,
        attachedDatabases: databaseNames.filter(
          name => name !== options.namespace
        ),
      }),
    },
...
function sqliteTrigger(
  ...
  const target =
    targetName === undefined ? undefined : relations.get(targetName)
  const beginIndex = sqlText?.search(/\bBEGIN\b/i) ?? -1
  const header =
    sqlText === undefined || beginIndex < 0
      ? undefined
      : sqlText.slice(0, beginIndex)
...
function sqliteExtension(
  ...
  return { dialect: 'sqlite', version: 1, data }
}

function indexExpressionFor(
  sqlText: string | undefined,
  position: number
): string | undefined {
  if (!sqlText) return undefined
...
function opaqueIndexExpression(
  ...
  return {
    kind: 'opaque-object',
    id: stableId(
      `index-expression:${table.physicalName}:${indexName}:${position}`
    ),
...
function table(
  ...
  return {
    ...
    unknownFields: [
      ...(withoutRowid ? [{ name: 'withoutRowid', value: true as const }] : []),
      ...(strict ? [{ name: 'strict', value: true as const }] : []),
...
function generatedExpressionFor(
  ...
  for (const definition of definitions) {
    ...
    if (!generated || generated.index === undefined) return undefined
    const generatedOpen = definition.indexOf('(', generated.index)
    if (generatedOpen < 0) return undefined
