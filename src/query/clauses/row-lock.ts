...
export function rowLock<TMode extends RowLockMode>(mode: TMode): RowLockClause<TMode, "default">
export function rowLock<TMode extends RowLockMode, TWait extends RowLockWaitPolicy>(
  mode: TMode,
  wait: TWait,
): RowLockClause<TMode, TWait>
...
function validateRowLockMode(mode: unknown): asserts mode is RowLockMode {
  if (rowLockModes.includes(mode as RowLockMode)) {
    return
  }
...
function validateRowLockWaitPolicy(wait: unknown): asserts wait is RowLockWaitPolicy {
  if (rowLockWaitPolicies.includes(wait as RowLockWaitPolicy)) {
    return
  }
