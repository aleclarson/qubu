...
export interface WhereClause<TMetadata = never> extends SelectClause<TMetadata> {
  readonly clauseKind: "where"
  readonly condition: BooleanExpression<any>
}

type WhereComposition<TCondition extends BooleanExpression<any> | Omit> = TCondition extends Omit
  ? Omit
...
export function where<TCondition extends BooleanExpression<any> | Omit>(
  ...
  if (condition === omit) {
    return omit as WhereComposition<TCondition>
  }
