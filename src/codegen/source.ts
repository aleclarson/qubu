...
export function printSchemaSource(
  ...
  const tables = schema.tables.map(table =>
    printTable(schema, table, diagnostics)
  )
...
function printTable(
  ...
  const columns = table.columns.map(column =>
    printColumn(schema, table, column, diagnostics)
  )
  ...
  const indexes = table.indexes.map(index =>
    printIndex(schema, table, index, diagnostics)
  )
  if (
...
function printConstraintType(
  ...
  const columns = printColumnTypeTuple(
    table,
    snapshot.columns,
    columnsName,
    path,
...
function printIndexTermType(
  ...
  if (term.kind !== 'order') {
    return printIndexTermExpressionType(
      table,
      term,
      columnsName,
      path,
...
function printIndexTermExpressionType(
  ...
  if (column === undefined) {
    diagnostics.push(
      sourceError(`Index column "${expression.column}" was not resolved`, [
        ...path,
        'column',
      ])
    )
...
function printColumnTypeTuple(
  ...
  for (const [position, id] of ids.entries()) {
    ...
    if (column === undefined) {
      diagnostics.push(
        sourceError(`Column "${id}" was not resolved`, [
          ...path,
          'columns',
          position,
        ])
...
function printColumn(
  ...
  if (snapshot.storage?.kind !== 'native') {
    diagnostics.push(
      sourceError('Generated columns require native storage', [
        ...path,
        'storage',
      ])
    )
  ...
  if (snapshot.default !== undefined) {
    if (snapshot.default.kind === 'external') {
      ...
      const value = printSnapshotLiteral(
...
function printConstraint(
  ...
  if (snapshot.kind === 'check') {
    ...
    if (targetTable === undefined) {
      diagnostics.push(
        sourceError(
          `Foreign-key target table "${snapshot.target.table}" was not resolved`,
          [...path, 'target', 'table']
        )
      )
      return undefined
  ...
  switch (value.kind) {
    ...
    case 'number': {
      ...
      if (!Number.isFinite(number)) {
        diagnostics.push(
          sourceError('Default number literals must be finite', [
            ...path,
            'default',
          ])
        )
