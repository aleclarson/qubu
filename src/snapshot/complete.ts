      ...
      diagnostics.map(issue =>
        Object.freeze({
          ...
          relatedPaths: issue.relatedPaths
            ? Object.freeze(
                issue.relatedPaths.map(path => Object.freeze([...path]))
              )
            : undefined,
        })
      )
...
export function encodeCompleteSchemaSnapshot(
...
export function completeSchemaSnapshotDigest(
  ...
  const source = encodeCompleteSchemaSnapshot(
    snapshot as CompleteSchemaSnapshot
  )
...
function validateCompleteSnapshot(
  ...
  if (value.format !== completeSchemaSnapshotFormat)
    diagnostics.push(
      issue(
        'invalid-snapshot',
        `Snapshot format must be "${completeSchemaSnapshotFormat}"`,
        ['format']
      )
    )
    ...
    diagnostics.push(
      issue(
        typeof value.version === 'number' &&
          value.version > completeSchemaSnapshotVersion
          ? 'future-version'
          : 'invalid-snapshot',
  ...
  const namingPolicy = validateNamingPolicy(
    value.namingPolicy,
    ['namingPolicy'],
    diagnostics
  )
...
function validateDialect(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Dialect must be an object', path)
    )
    return undefined
  }
  ...
  if (version !== undefined && version < 1)
    diagnostics.push(
      issue('invalid-snapshot', 'Dialect version must be positive', [
        ...path,
        'version',
      ])
    )
  if (version !== undefined && version > schemaSnapshotDialectVersion)
    diagnostics.push(
      issue('future-version', `Unsupported dialect version: ${version}`, [
        ...path,
        'version',
      ])
    )
...
function validateNamingPolicy(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Naming policy must be an object', path)
    )
    return undefined
  }
  requireKeys(value, ['name', 'version'], path, diagnostics)
...
function validateNamespace(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Snapshot namespace must be an object', path)
    )
    return undefined
  }
  requireKeys(
  ...
  if (
    ...
    diagnostics.push(
      issue('invalid-snapshot', 'Snapshot namespace kind is invalid', [
        ...path,
        'kind',
      ])
    )
...
function validateCapabilities(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Snapshot capabilities must be an object', path)
    )
    return undefined
  }
  const required = [
  ...
  for (const key of Object.keys(value).sort()) {
    ...
    if (typeof child !== 'boolean' && typeof child !== 'string')
      diagnostics.push(
        issue('invalid-value', 'Capability values must be boolean or string', [
          ...path,
          key,
        ])
      )
  ...
  if (
    ...
    diagnostics.push(
      issue('invalid-snapshot', 'Capability visibility is invalid', [
        ...path,
        'visibility',
      ])
    )
  for (const key of required) {
    if (
      ...
      diagnostics.push(
        issue('invalid-snapshot', 'Capability must be boolean', [...path, key])
      )
  }
  if (
    diagnostics.some(
...
function validateObjects<T>(
  ...
  for (const [index, item] of value.entries()) {
    ...
    if (valid !== undefined) result.push(valid)
  }
  return result
}
...
function validateBase(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', `${kind} must be an object`, path)
    )
    return undefined
  }
  requireKeys(
  ...
  if (
    value.kind !== kind &&
    !(kind === 'view' && value.kind === 'materialized-view')
  )
    diagnostics.push(
      issue('invalid-snapshot', `${kind} kind is invalid`, [...path, 'kind'])
    )
  const id = stringValue(value.id, [...path, 'id'], diagnostics, true)
  const physicalName = stringValue(
    value.physicalName,
    [...path, 'physicalName'],
    diagnostics,
    true
...
function validateTable(
  ...
  if (!isRecord(value) || base === undefined) return undefined
...
function validateColumn(
  ...
  const nullable = booleanValue(
    value.nullable,
    [...path, 'nullable'],
    diagnostics
  )
  const hasDefault = booleanValue(
    value.hasDefault,
    [...path, 'hasDefault'],
    diagnostics
  )
  const generated = booleanValue(
    value.generated,
    [...path, 'generated'],
    diagnostics
  )
...
function validateConstraint(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Constraint must be an object', path)
    )
    return undefined
  }
  const kind = value.kind
  ...
  if (kind === 'check') {
    ...
    const validated = optionalBoolean(
      value.validated,
      [...path, 'validated'],
      diagnostics
    )
  ...
  if (kind === 'foreign-key') {
    const columns = validateIds(
      value.columns,
      [...path, 'columns'],
      diagnostics
    )
    const target = validateForeignKeyTarget(
      value.target,
      [...path, 'target'],
      diagnostics
    )
    const onUpdate = validateAction(
      value.onUpdate,
      [...path, 'onUpdate'],
      diagnostics
    )
    const onDelete = validateAction(
      value.onDelete,
      [...path, 'onDelete'],
      diagnostics
    )
  ...
  if (
    ...
    diagnostics.push(
      issue('invalid-snapshot', 'Constraint kind is invalid', [...path, 'kind'])
    )
    return undefined
  }
  ...
  if (kind === 'unique-constraint' && nulls === undefined) {
    diagnostics.push(
      issue('invalid-snapshot', 'Unique constraints require nulls semantics', [
        ...path,
        'nulls',
      ])
    )
...
function validateIndex(
  ...
  const candidateKey = booleanValue(
    value.candidateKey,
    [...path, 'candidateKey'],
    diagnostics
  )
...
function validateIndexTerm(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Index term must be an object', path)
    )
    return undefined
  }
  const kind = value.kind
  ...
  if (kind === 'column') {
    ...
    if (column === undefined || position === undefined) return undefined
    return {
      kind: 'column',
      column,
      position,
  ...
  if (kind === 'expression') {
    ...
    if (expression === undefined || position === undefined) return undefined
    return {
      kind: 'expression',
      expression,
      position,
  ...
  diagnostics.push(
    issue('invalid-snapshot', 'Index term kind is invalid', [...path, 'kind'])
  )
  return undefined
}
...
function validateView(
  ...
  const checkOption = validateCheckOption(
    value.checkOption,
    [...path, 'checkOption'],
    diagnostics
  )
  ...
  if (columns === undefined || definition === undefined) return undefined
  return {
...
function validateEnum(
  ...
  const base = validateBase(value, path, 'enum', undefined, diagnostics, [
    'values',
  ])
  if (!isRecord(value) || base === undefined) return undefined
  ...
  for (const [index, item] of values.entries()) {
    if (item.ordinalPosition <= previous)
      diagnostics.push(
        issue(
          'non-canonical',
          'Enum labels must be sorted by ordinal position',
          [...path, 'values', index, 'ordinalPosition']
        )
      )
...
function validateEnumValue(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Enum value must be an object', path)
    )
    return undefined
  }
  requireKeys(
    value,
    ['value', 'ordinalPosition', 'provenance'],
    path,
    diagnostics,
    ['provenance']
  )
...
function validateCheckConstraint(
  ...
  if (constraint?.kind !== 'check') {
    diagnostics.push(
      issue(
        'invalid-snapshot',
        'Domain constraints must be check constraints',
        path
      )
    )
    return undefined
...
function validateTrigger(
  ...
  if (!isRecord(value) || base === undefined) return undefined
  const table = validateObjectReference(
    value.table,
    [...path, 'table'],
    diagnostics
  )
  const timing = validateTimingKind(
    value.timing,
    [...path, 'timing'],
    diagnostics
  )
...
function validateRoutine(
  ...
  if (!isRecord(value) || base === undefined) return undefined
  const routineKind = validateRoutineKind(
    value.routineKind,
    [...path, 'routineKind'],
    diagnostics
  )
  ...
  if (routineKind === undefined || parameters === undefined) return undefined
  return {
    kind: 'routine',
    id: base.id,
    physicalName: base.physicalName,
...
function validateRoutineParameter(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Routine parameter must be an object', path)
    )
    return undefined
  }
  requireKeys(
    value,
    ['name', 'mode', 'storage', 'default', 'ordinalPosition'],
    path,
    diagnostics,
    ['name', 'mode', 'default']
  )
  const name =
    value.name === undefined
      ? undefined
      : stringValue(value.name, [...path, 'name'], diagnostics)
...
function validatePartition(
  ...
  if (!isRecord(value) || base === undefined) return undefined
  const parent = validateObjectReference(
    value.parent,
    [...path, 'parent'],
    diagnostics
  )
  ...
  const defaultValue = optionalBoolean(
    value.default,
    [...path, 'default'],
    diagnostics
  )
  if (parent === undefined || strategy === undefined) return undefined
  return {
    kind: 'partition',
    id: base.id,
    physicalName: base.physicalName,
...
function validatePolicy(
  ...
  const permissive = optionalBoolean(
    value.permissive,
    [...path, 'permissive'],
    diagnostics
  )
  ...
  if (table === undefined || command === undefined) return undefined
  return {
    kind: 'policy',
    id: base.id,
    physicalName: base.physicalName,
...
function validateExtensionObject(
  ...
  if (extensionName === undefined || data === undefined) return undefined
  return {
    kind: 'extension',
    id: base.id,
    physicalName: base.physicalName,
...
function validateDeferredObject(
  ...
  const base = validateBase(
    ...
    undefined,
    diagnostics,
    ['objectKind', 'reason', 'data']
  )
  if (!isRecord(value) || base === undefined) return undefined
  const objectKind = stringValue(
    value.objectKind,
    [...path, 'objectKind'],
    diagnostics,
    true
...
function validateOpaqueObject(
  ...
  if (objectKind === undefined || data === undefined) return undefined
  return {
    kind: 'opaque-object',
    id: base.id,
    objectKind,
...
function validateComment(
  ...
  const base = validateBase(value, path, 'comment', undefined, diagnostics, [
    'object',
    'text',
  ])
  if (!isRecord(value) || base === undefined) return undefined
  const object = validateObjectReference(
    value.object,
    [...path, 'object'],
    diagnostics
  )
  const text = stringValue(value.text, [...path, 'text'], diagnostics)
  if (object === undefined || text === undefined) return undefined
  return {
    kind: 'comment',
    id: base.id,
    physicalName: base.physicalName,
...
function validateOwnership(
  ...
  const base = validateBase(value, path, 'ownership', undefined, diagnostics, [
    'object',
    'owner',
  ])
  if (!isRecord(value) || base === undefined) return undefined
  ...
  if (object === undefined || owner === undefined) return undefined
  return {
    kind: 'ownership',
    id: base.id,
    physicalName: base.physicalName,
...
function validateStorage(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Storage must be an object', path)
    )
    return undefined
  }
  ...
  if (value.kind === 'native') {
    ...
    if (dialect === undefined || type === undefined) return undefined
    return {
      kind: 'native',
      dialect,
      type,
  ...
  diagnostics.push(
    issue('invalid-snapshot', 'Storage kind is invalid', [...path, 'kind'])
  )
  return undefined
}
...
function validateDefault(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Default must be an object', path)
    )
    return undefined
  }
  ...
  diagnostics.push(
    issue('invalid-snapshot', 'Default kind is invalid', [...path, 'kind'])
  )
  return undefined
}
...
function validateGenerated(
  ...
  if (value.kind === 'expression') {
    ...
    return expression === undefined || mode === undefined
      ? undefined
      : { kind: 'expression' as const, expression, mode }
  }
  diagnostics.push(
    issue('invalid-snapshot', 'Generated-column kind is invalid', [
      ...path,
      'kind',
    ])
  )
...
function validateIdentity(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Identity must be an object', path)
    )
    return undefined
  }
  requireKeys(
  ...
  if (value.kind !== 'identity')
    diagnostics.push(
      issue('invalid-snapshot', 'Identity kind must be identity', [
        ...path,
        'kind',
      ])
    )
  ...
  const options = validateValueFactRecord(
    value.options,
    [...path, 'options'],
    diagnostics
  )
  ...
  if (
    ...
    return undefined
  return { kind: 'identity', generation, options, ...metadata }
}
...
function validateExpression(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Expression must be an object', path)
    )
    return undefined
  }
  requireKeys(
    value,
    ['kind', 'expressionKind', 'sql', 'dialect'],
    path,
    diagnostics,
    ['dialect']
  )
  ...
  if (value.kind !== 'expression')
    diagnostics.push(
      issue('invalid-snapshot', 'Expression kind must be expression', [
        ...path,
        'kind',
      ])
    )
  if (sql !== undefined && sql !== sql.replace(/\r\n?/g, '\n'))
    diagnostics.push(
      issue('non-canonical', 'Expression SQL must use LF line endings', [
        ...path,
        'sql',
      ])
    )
  ...
  if (
    ...
    diagnostics.push(
      issue(
        'invalid-snapshot',
        'Only unsafe expressions may carry a dialect tag',
        [...path, 'dialect']
      )
    )
  if (
...
function validateLiteral(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Literal must be an object', path)
    )
    return undefined
  }
  if (value.kind === 'null') {
  ...
  if (value.kind === 'boolean' || value.kind === 'string') {
    ...
    if (typeof value.value !== expected)
      diagnostics.push(
        issue('invalid-snapshot', `${expected} literal value is invalid`, [
          ...path,
          'value',
        ])
      )
    return typeof value.value === expected
      ? ({ kind: value.kind, value: value.value } as SnapshotLiteral)
      : undefined
  }
  if (value.kind === 'number' || value.kind === 'bigint') {
    ...
    const valid =
      typeof raw === 'string' && isCanonicalNumber(raw, value.kind === 'bigint')
    if (!valid)
      diagnostics.push(
        issue(
          'invalid-snapshot',
          `${value.kind} literal value is not canonical`,
          [...path, 'value']
        )
      )
    return valid ? { kind: value.kind, value: raw as string } : undefined
  }
  diagnostics.push(
    issue('invalid-snapshot', 'Literal kind is invalid', [...path, 'kind'])
  )
  return undefined
}
...
function validateValueFact(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Value fact must be an object', path)
    )
    return undefined
  }
  if (value.kind === 'literal') {
  ...
  diagnostics.push(
    issue('invalid-snapshot', 'Value fact kind is invalid', [...path, 'kind'])
  )
  return undefined
}
...
function validateValueFactRecord(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Value-fact options must be an object', path)
    )
    return undefined
  }
...
function validateProvenance(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Provenance must be an object', path)
    )
    return undefined
  }
  ...
  const sourceDialect = stringValue(
    value.dialect,
    [...path, 'dialect'],
    diagnostics,
    true
  ...
  const sourcePath =
    value.path === undefined
      ? undefined
      : validatePath(value.path, [...path, 'path'], diagnostics)
  if (
    dialect !== undefined &&
  ...
  if (kind === undefined || sourceDialect === undefined) return undefined
  return {
    kind,
...
function validatePhysicalReference(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Physical reference must be an object', path)
    )
    return undefined
  }
  requireKeys(
    value,
    ['kind', 'namespace', 'table', 'name'],
    path,
    diagnostics,
    ['namespace', 'table']
  )
  ...
  if (
    ...
    diagnostics.push(
      issue('invalid-snapshot', `Unknown physical reference kind: ${kind}`, [
        ...path,
        'kind',
      ])
    )
...
function validateExtension(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Dialect extension must be an object', path)
    )
    return undefined
  }
  requireKeys(value, ['dialect', 'version', 'data'], path, diagnostics)
  const extensionDialect = stringValue(
    value.dialect,
    [...path, 'dialect'],
    diagnostics,
    true
  ...
  if (version !== undefined && version < 1)
    diagnostics.push(
      issue('invalid-snapshot', 'Dialect extension version must be positive', [
        ...path,
        'version',
      ])
    )
  if (version !== undefined && version > schemaSnapshotDialectVersion)
    diagnostics.push(
      issue(
        'future-version',
        `Unsupported dialect extension version: ${version}`,
        [...path, 'version']
      )
    )
  if (
    dialect !== undefined &&
    extensionDialect !== undefined &&
    extensionDialect !== dialect
  )
    diagnostics.push(
      issue(
        'dialect-mismatch',
  ...
  if (
    ...
    return undefined
  return { dialect: extensionDialect, version, data }
}
...
function validateJsonValue(
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value))
      diagnostics.push(
        issue(
          'invalid-value',
          'Non-finite JSON numbers must use a tagged value',
          path
        )
      )
  ...
  if (typeof value !== 'object') {
    diagnostics.push(
      issue(
        'invalid-value',
        'Snapshot data contains an unsupported value',
        path
      )
    )
    return undefined
  ...
  if (seen.has(value)) {
    diagnostics.push(
      issue('invalid-value', 'Snapshot data cannot be cyclic', path)
    )
    return undefined
  }
  seen.add(value)
  try {
    ...
    if (!isRecord(value)) {
      diagnostics.push(
        issue('invalid-value', 'Snapshot data must use plain objects', path)
      )
      return undefined
    }
    if (Object.keys(value).length === 1 && '$bigint' in value) {
      if (
        ...
        !isCanonicalNumber(value.$bigint, true)
      )
        diagnostics.push(
          issue('invalid-value', 'Tagged bigint is not canonical', [
            ...path,
            '$bigint',
          ])
        )
    ...
    if (Object.keys(value).length === 1 && '$number' in value) {
      if (
        ...
        diagnostics.push(
          issue('invalid-value', 'Tagged number is invalid', [
            ...path,
            '$number',
          ])
        )
...
function validateObjectReference(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Object reference must be an object', path)
    )
    return undefined
  }
  ...
  if (kind === 'namespace')
    diagnostics.push(
      issue('invalid-snapshot', 'Object references cannot target a namespace', [
        ...path,
        'kind',
      ])
    )
  if (kind !== undefined && !completeObjectKinds.has(kind))
    diagnostics.push(
      issue('invalid-snapshot', `Unknown object reference kind: ${kind}`, [
        ...path,
        'kind',
      ])
    )
  return kind === undefined ||
    ...
    kind === 'namespace'
    ? undefined
    : { kind: kind as CompleteSnapshotObjectReference['kind'], id }
}
...
function validateForeignKeyTarget(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Foreign-key target must be an object', path)
    )
    return undefined
  }
...
function validateIds(
  ...
  for (const [index, id] of result.entries()) {
    if (seen.has(id))
      diagnostics.push(
        issue('non-canonical', 'References cannot contain duplicate IDs', [
          ...path,
          index,
        ])
      )
...
function validateStrings(
  ...
  for (const [index, item] of value.entries()) {
    ...
    if (string !== undefined) result.push(string)
  }
...
function validateTiming(
  ...
  const deferrable = optionalBoolean(
    value.deferrable,
    [...path, 'deferrable'],
    diagnostics
  )
...
function validateNulls(
  ...
  return validateChoice(
    value,
    ['distinct', 'not-distinct'] as const,
    path,
    diagnostics
  )
}
...
function validateEvents(
  ...
  for (const [index, event] of events.entries()) {
    if (!allowed.has(event))
      diagnostics.push(
        issue('invalid-snapshot', 'Trigger event is invalid', [...path, index])
      )
    else result.push(event as 'insert' | 'update' | 'delete' | 'truncate')
  }
  const sorted = [...result].sort()
  if (sorted.some((event, index) => event !== result[index]))
    diagnostics.push(
      issue('non-canonical', 'Trigger events must be sorted', path)
    )
  return result
}
...
function validatePath(
  ...
  if (!Array.isArray(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Provenance path must be an array', path)
    )
    return undefined
  }
  ...
  for (const [index, part] of value.entries()) {
    if (
      ...
      diagnostics.push(
        issue(
          'invalid-snapshot',
          'Provenance path parts must be strings or safe integers',
          [...path, index]
        )
      )
...
function integerValue(
  ...
  if (typeof value !== 'number' || !Number.isSafeInteger(value)) {
    diagnostics.push(
      issue('invalid-snapshot', 'Value must be a safe integer', path)
    )
    return undefined
  }
  return value
...
function validateCompleteDialectMetadata(
  ...
  for (const object of objects) {
    if (
      object.dialect?.dialect !== undefined &&
      object.dialect.dialect !== dialect
    )
      diagnostics.push(
        issue(
          'dialect-mismatch',
    ...
    if (
      object.provenance?.dialect !== undefined &&
      object.provenance.dialect !== dialect
    )
      diagnostics.push(
        issue(
          'dialect-mismatch',
...
function validateCompleteCrossReferences(
      ...
      if (constraint.kind === 'foreign-key') {
        ...
        if (target?.kind === 'table') {
          ...
          for (const [
            columnIndex,
            column,
          ] of constraint.target.columns.entries())
  ...
  for (const [index, routine] of snapshot.routines.entries())
    for (const [dependency, reference] of (
      ...
      diagnostics.push(
        issue(
          ...
          [
            ...path,
            index,
            item.position === undefined ? 'ordinalPosition' : 'position',
          ]
        )
      )
...
function requireKeys(
  ...
  for (const key of required)
    if (!optional.includes(key) && !(key in value))
      diagnostics.push(
        issue('invalid-snapshot', `Missing required field "${key}"`, [
          ...path,
          key,
        ])
      )
  for (const key of Object.keys(value))
    if (!allowed.has(key))
      diagnostics.push(
        issue('unknown-field', `Unknown snapshot field "${key}"`, [
          ...path,
          key,
        ])
      )
...
function issue(
  ...
  return { code, message, path: Object.freeze([...path]) }
}
