...
export type SqliteStorageAffinity = "blob" | "integer" | "numeric" | "real" | "text"

class SqliteSnapshotDialectError extends TypeError {
  readonly code = "dialect-mismatch" as const
}

const sqliteStorageTypes: Readonly<Record<PortableStorageType, string>> = Object.freeze({
  integer: "INTEGER",
  numeric: "NUMERIC",
  text: "TEXT",
  boolean: "INTEGER",
...
export function sqliteStorageAffinity(declaration: string): SqliteStorageAffinity {
  ...
  if (upper.includes("BLOB") || upper.trim().length === 0) {
    return "blob"
  }
...
export const sqliteSchemaDialect: SchemaDialect<"json" | "on-conflict"> = createSchemaDialect(
  sqliteDialect(),
  {
    version: schemaSnapshotDialectVersion,
...
export const sqliteSnapshotAdapter: SchemaSnapshotAdapter<"json" | "on-conflict"> = Object.freeze({
...
function encodeSqliteStorage(storage: ColumnStorage, dialect: SchemaDialect): SnapshotStorage {
  ...
  if (type === undefined) {
    throw new TypeError(`Unsupported portable storage type for SQLite: ${String(storage.type)}`)
  }
...
function encodeSqliteExtension(
  ...
  if (extension.dialect !== dialect.name) {
    throw new TypeError(`SQLite schema extensions require dialect "${dialect.name}"`)
  }
