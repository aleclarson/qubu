...
export function decodeSchemaSnapshot(
  ...
  if (!isRecord(value)) {
    return {
      ok: false,
      diagnostics: [
        diagnostic('invalid-snapshot', 'Snapshot root must be an object', []),
      ],
    }
  }
...
export function assertSchemaSnapshot(
  input: SchemaSnapshotInput | string
): SchemaSnapshot {
...
export function canonicalizeSchemaSnapshot(
...
function validateSnapshot(
  ...
  if (value.format !== schemaSnapshotFormat) {
    diagnostics.push(
      diagnostic(
        'invalid-snapshot',
        `Snapshot format must be "${schemaSnapshotFormat}"`,
        ['format']
      )
    )
  }
    ...
    diagnostics.push(
      diagnostic(
        typeof value.version === 'number' &&
          value.version > schemaSnapshotVersion
          ? 'future-version'
          : 'invalid-snapshot',
  ...
  if (value.namespace !== undefined) {
    if (
      ...
      diagnostics.push(
        diagnostic(
          'invalid-snapshot',
          'Snapshot namespace must be a non-empty identifier',
          ['namespace']
        )
      )
  }
  ...
  if (!Array.isArray(tablesValue)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Snapshot tables must be an array', [
        'tables',
      ])
    )
  } else {
    ...
    for (const [index, table] of tablesValue.entries()) {
      const validated = validateTable(
        table,
        ['tables', index],
        dialect?.name,
        diagnostics
      )
      if (validated !== undefined) tables.push(validated)
    }
  }

  if (dialect === undefined || namingPolicy === undefined) return undefined
...
function validateDialect(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Dialect must be an object', path)
    )
    return undefined
  }
  ...
  if (typeof value.name !== 'string' || value.name.length === 0) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Dialect name must be non-empty', [
        ...path,
        'name',
      ])
    )
  ...
  if (
    ...
    diagnostics.push(
      diagnostic(
        'invalid-snapshot',
        'Dialect version must be a positive integer',
        [...path, 'version']
      )
    )
    ...
    diagnostics.push(
      diagnostic(
        'future-version',
        `Unsupported dialect extension version: ${value.version}`,
        [...path, 'version']
      )
    )
  }
...
function validateNamingPolicy(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Naming policy must be an object', path)
    )
    return undefined
  }
  requireKeys(value, ['name', 'version'], path, diagnostics)
  if (typeof value.name !== 'string' || value.name.length === 0) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Naming-policy name must be non-empty', [
        ...path,
        'name',
      ])
    )
  ...
  if (value.version !== schemaSnapshotNamingPolicyVersion) {
    diagnostics.push(
      diagnostic(
        typeof value.version === 'number' &&
          value.version > schemaSnapshotNamingPolicyVersion
          ? 'future-version'
          : 'invalid-snapshot',
...
function validateTable(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Table must be an object', path)
    )
    return undefined
  }
  requireKeys(
    value,
    ['id', 'physicalName', 'columns', 'constraints', 'indexes'],
    path,
    diagnostics
  )
  ...
  if (
    ...
    return undefined
  return { id, physicalName, columns, constraints, indexes }
}
...
function validateColumn(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Column must be an object', path)
    )
    return undefined
  }
  requireKeys(
  ...
  const nullable = validateBoolean(
    value.nullable,
    [...path, 'nullable'],
    diagnostics
  )
  const hasDefault = validateBoolean(
    value.hasDefault,
    [...path, 'hasDefault'],
    diagnostics
  )
  const generated = validateBoolean(
    value.generated,
    [...path, 'generated'],
    diagnostics
  )
...
function validateStorage(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Storage must be an object', path)
    )
    return undefined
  }
...
function validateDefault(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Default must be an object', path)
    )
    return undefined
  }
  ...
  diagnostics.push(
    diagnostic('invalid-snapshot', 'Default kind is invalid', [...path, 'kind'])
  )
  return undefined
}
...
function validateGenerated(
  ...
  if (value.kind === 'expression') {
    ...
    const mode =
      value.mode === 'stored' || value.mode === 'virtual'
        ? value.mode
        : undefined
    if (mode === undefined)
      diagnostics.push(
    ...
    return expression === undefined || mode === undefined
      ? undefined
      : { kind: 'expression', expression, mode }
  }
  diagnostics.push(
    diagnostic('invalid-snapshot', 'Generated-column kind is invalid', [
      ...path,
      'kind',
    ])
  )
...
function validateIdentity(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Identity must be an object', path)
    )
    return undefined
  }
  ...
  if (value.generation !== 'always' && value.generation !== 'by-default')
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Identity generation is invalid', [
        ...path,
        'generation',
      ])
    )
...
function validateConstraint(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Constraint must be an object', path)
    )
    return undefined
  }
  ...
  if (value.kind === 'primary-key' || value.kind === 'unique') {
    ...
    const columns = validateColumnIds(
      value.columns,
      [...path, 'columns'],
      diagnostics
    )
    ...
    if (columns === undefined || extra === undefined) return undefined
    return {
      ...common,
      kind: value.kind,
      columns,
  ...
  if (value.kind === 'unique-constraint') {
    ...
    if (value.nulls !== 'distinct' && value.nulls !== 'not-distinct')
      diagnostics.push(
        diagnostic(
          'invalid-snapshot',
          'Unique-constraint null semantics are invalid',
          [...path, 'nulls']
        )
      )
    if (
  ...
  if (value.kind === 'check') {
    ...
    if (expression === undefined || extra === undefined) return undefined
    return {
      ...common,
      kind: 'check',
      expression,
  ...
  diagnostics.push(
    diagnostic(
      'invalid-snapshot',
      `Unknown constraint kind: ${String(value.kind)}`,
      [...path, 'kind']
    )
  )
  return undefined
...
function validateConstraintCommon(
  ...
  | { id: string; kind: SnapshotConstraint['kind']; physicalName: string }
  | undefined {
  const id = validateId(value.id, [...path, 'id'], diagnostics)
  const physicalName = validateName(
    value.physicalName,
  ...
  if (id === undefined || physicalName === undefined) return undefined
  return { id, kind: value.kind as SnapshotConstraint['kind'], physicalName }
}
...
function validateConstraintTiming(
  ...
  path: readonly (string | number)[],
  diagnostics: SnapshotDiagnostic[]
): { deferrable?: boolean; initially?: 'immediate' | 'deferred' } | undefined {
  const deferrable = validateOptionalBoolean(
    value.deferrable,
    [...path, 'deferrable'],
    diagnostics
  )
  ...
  if (value.initially !== undefined && initially === undefined)
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Constraint timing is invalid', [
        ...path,
        'initially',
      ])
    )
  if (deferrable === undefined && value.deferrable !== undefined)
    return undefined
  if (value.initially !== undefined && initially === undefined) return undefined
  return {
...
function validateForeignKeyOptions(
  ...
  const action = (key: 'onUpdate' | 'onDelete') => {
    ...
    if (
      ...
      return candidate
    diagnostics.push(
      diagnostic('invalid-snapshot', `Foreign-key ${key} action is invalid`, [
        ...path,
        key,
      ])
    )
  ...
  if (value.match !== undefined && match === undefined)
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Foreign-key match mode is invalid', [
        ...path,
        'match',
      ])
    )
...
function validateIndex(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Index must be an object', path)
    )
    return undefined
  }
  requireKeys(
  ...
  if (value.kind !== 'index')
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Index kind must be index', [
        ...path,
        'kind',
      ])
    )
  const unique = validateBoolean(value.unique, [...path, 'unique'], diagnostics)
  const candidateKey = validateBoolean(
    value.candidateKey,
    [...path, 'candidateKey'],
    diagnostics
  )
...
function validateIndexTerm(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Index term must be an object', path)
    )
    return undefined
  }
  if (value.kind === 'column') {
    ...
    return column === undefined ? undefined : { kind: 'column', column }
  }
  if (value.kind === 'expression') {
    requireKeys(value, ['kind', 'expression'], path, diagnostics)
    const expression = validateExpression(
  ...
  if (value.kind === 'order') {
    requireKeys(
      value,
      ['kind', 'expression', 'direction', 'nulls'],
      path,
      diagnostics,
      ['direction', 'nulls']
    )
    ...
    if (value.direction !== undefined && direction === undefined)
      diagnostics.push(
        diagnostic('invalid-snapshot', 'Index order direction is invalid', [
          ...path,
          'direction',
        ])
      )
    if (value.nulls !== undefined && nulls === undefined)
      diagnostics.push(
        diagnostic('invalid-snapshot', 'Index NULL ordering is invalid', [
          ...path,
          'nulls',
        ])
      )
  ...
  diagnostics.push(
    diagnostic('invalid-snapshot', 'Index term kind is invalid', [
      ...path,
      'kind',
    ])
  )
...
function validateIndexTermExpression(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Index expression must be an object', path)
    )
    return undefined
  }
  if (value.kind === 'column') {
  ...
  diagnostics.push(
    diagnostic('invalid-snapshot', 'Index expression kind is invalid', [
      ...path,
      'kind',
    ])
  )
...
function validateExpression(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Expression must be an object', path)
    )
    return undefined
  }
  requireKeys(
  ...
  if (sql === undefined)
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Expression SQL must be a string', [
        ...path,
        'sql',
      ])
    )
  if (value.kind !== 'expression')
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Expression kind must be expression', [
        ...path,
        'kind',
      ])
    )
  ...
  if (expressionDialect === undefined && value.expressionKind === 'unsafe')
    diagnostics.push(
      diagnostic(
        'invalid-snapshot',
        'Unsafe expressions must carry a dialect tag',
        [...path, 'dialect']
      )
    )
  if (sql !== undefined && sql !== sql.replace(/\r\n?/g, '\n'))
    diagnostics.push(
      diagnostic('non-canonical', 'Expression SQL must use LF line endings', [
        ...path,
        'sql',
      ])
    )
...
function validateLiteral(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Literal must be an object', path)
    )
    return undefined
  }
  if (value.kind === 'null') {
  ...
  if (value.kind === 'boolean') {
    ...
    if (typeof value.value !== 'boolean')
      diagnostics.push(
        diagnostic(
          'invalid-snapshot',
          'Boolean literal value must be boolean',
          [...path, 'value']
        )
      )
  ...
  if (value.kind === 'string') {
    ...
    if (typeof value.value !== 'string')
      diagnostics.push(
        diagnostic('invalid-snapshot', 'String literal value must be string', [
          ...path,
          'value',
        ])
      )
  ...
  diagnostics.push(
    diagnostic('invalid-snapshot', 'Literal kind is invalid', [...path, 'kind'])
  )
  return undefined
}
...
function validateExtension(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic(
        'invalid-snapshot',
        'Dialect extension must be an object',
        path
      )
    )
    return undefined
  ...
  const extensionDialect = validateName(
    value.dialect,
    [...path, 'dialect'],
    diagnostics
  )
  ...
  if (
    ...
    diagnostics.push(
      diagnostic(
        'invalid-snapshot',
        'Dialect extension version must be a positive integer',
        [...path, 'version']
      )
    )
...
function validateJsonValue(
  ...
  if (typeof value !== 'object') {
    diagnostics.push(
      diagnostic(
        'invalid-value',
        'Dialect data contains an unsupported value',
        path
      )
    )
    return undefined
  ...
  if (seen.has(value)) {
    diagnostics.push(
      diagnostic('invalid-value', 'Dialect data cannot be cyclic', path)
    )
    return undefined
  }
  seen.add(value)
  try {
    ...
    if (!isRecord(value)) {
      diagnostics.push(
        diagnostic('invalid-value', 'Dialect data must use plain objects', path)
      )
      return undefined
    }
    if (Object.keys(value).length === 1 && '$bigint' in value) {
      if (
        ...
        diagnostics.push(
          diagnostic('invalid-value', 'Tagged bigint is not canonical', [
            ...path,
            '$bigint',
          ])
        )
    ...
    if (Object.keys(value).length === 1 && '$number' in value) {
      if (
        ...
        diagnostics.push(
          diagnostic('invalid-value', 'Tagged number is invalid', [
            ...path,
            '$number',
          ])
        )
      ...
      if (constraint.kind === 'foreign-key') {
        ...
        if (target === undefined) {
          ...
          for (const [
            ...
            if (
              !target.columns.some(targetColumn => targetColumn.id === column)
            )
              diagnostics.push(
                diagnostic(
                  'invalid-cross-reference',
...
function validateColumnIds(
  ...
  if (!Array.isArray(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Column references must be an array', path)
    )
    return undefined
  }
  const result: string[] = []
  ...
  if (result.length === 0)
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Column references cannot be empty', path)
    )
  return result
}
...
function validateArray<T>(
  ...
  if (!Array.isArray(value)) {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Value must be an array', path)
    )
    return undefined
  }
  const result: T[] = []
  ...
  for (const [index, item] of value.entries()) {
    ...
    if (previous !== undefined && item.id <= previous)
      diagnostics.push(
        diagnostic(
          'non-canonical',
          'Entities must be sorted by stable logical ID',
          [...path, index, 'id']
        )
      )
    previous = item.id
...
function validateId(
  ...
  if (
    ...
    diagnostics.push(
      diagnostic(
        'invalid-snapshot',
        'Logical IDs must be non-empty identifiers',
        path
      )
    )
    return undefined
...
function validateBoolean(
  ...
  if (typeof value !== 'boolean') {
    diagnostics.push(
      diagnostic('invalid-snapshot', 'Value must be boolean', path)
    )
    return undefined
  }
  return value
...
function validateOptionalBoolean(
  ...
  return value === undefined
    ? undefined
    : validateBoolean(value, path, diagnostics)
}
...
function requireKeys(
  ...
  for (const key of required) {
    if (!(key in value) && !optional.includes(key))
      diagnostics.push(
        diagnostic('invalid-snapshot', `Missing required field "${key}"`, [
          ...path,
          key,
        ])
      )
  ...
  for (const key of Object.keys(value)) {
    if (!allowed.has(key))
      diagnostics.push(
        diagnostic('unknown-field', `Unknown snapshot field "${key}"`, [
          ...path,
          key,
        ])
      )
