  ...
  switch (typeof value) {
    ...
    case 'symbol':
      throw new SnapshotCanonicalError(
        `Snapshot values cannot contain ${typeof value}`,
        path
      )
    case 'object':
      break
    ...
    if (Array.isArray(value)) {
      return Object.freeze(
        value.map((item, index) =>
          toSnapshotJsonValue(item, [...path, index], seen)
        )
...
export function canonicalJson(value: SnapshotJsonValue): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) {
      throw new SnapshotCanonicalError(
        'Non-finite numbers must use a tagged snapshot number'
      )
    }
 ...
 */
export function schemaSnapshotDigest(
  snapshot: SchemaSnapshotInput | string
): string {
  const source = encodeCanonicalSnapshot(
    assertSchemaSnapshot(snapshot) as SchemaSnapshot
  )
