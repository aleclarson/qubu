...
function assertFileTarget(packageRoot, target, subpath, condition) {
  assert.match(target, /^\.\//, `${subpath} ${condition} target must start with "./"`)
...
function validateExports(packageRoot, exports, sourceExportKeys) {
  ...
  for (const [subpath, descriptor] of Object.entries(exports)) {
    assert(subpath === "." || subpath.startsWith("./"), `invalid package export key: ${subpath}`)
    ...
    assert.equal(typeof descriptor.types, "string", `${subpath} must declare a types target`)
    assert.match(
      descriptor.types,
      /^\.\/dist\/.+\.d\.[cm]?ts$/,
    ...
    if ("import" in descriptor) {
      ...
      assert.equal(typeof descriptor.import, "string", `${subpath} must declare an import target`)
      assert.match(
        descriptor.import,
        /^\.\/dist\/.+\.mjs$/,
  ...
  assert(runtimeSpecifiers.length > 0, "package must expose runtime entrypoints")
  assert(typeOnlySpecifiers.length > 0, "package must expose a type-only entrypoint")
...
function createTypeSmoke(consumerRoot, runtimeSpecifiers, typeOnlySpecifiers) {
  ...
  const typeOnlyImports = typeOnlySpecifiers
    ...
    .join("\n")
  const entries = runtimeSpecifiers.map((_, index) => `entry${index}`).join(", ")
...
function testPackedPackage(runtimes) {
  ...
  try {
    ...
    if (runtimes.includes("node")) {
      ...
      console.log(`Node loaded ${runtimeSpecifiers.length} packed runtime entrypoints.`)
    }

    if (runtimes.includes("bun")) {
      run("bun", [smokeFile], temporaryRoot)
      console.log(`Bun loaded ${runtimeSpecifiers.length} packed runtime entrypoints.`)
    }
  } finally {
...
const runtimeOption = process.argv.find((argument) => argument.startsWith("--runtime="))
...
if (checkBuilt) {
  ...
  testPackedPackage(runtime ? [runtime] : ["node", "bun"])
}
