...
assert.equal(
  ...
  0,
  `Unknown argument: ${unknownArguments.join(', ')}`
)
...
for (const [label, threshold] of Object.entries(thresholds.metrics)) {
  ...
  if (actual > threshold.limit) {
    failures.push(
      `${label} ${formatCount(actual)} exceeds ${formatCount(threshold.limit)}`
    )
  }
}
...
for (const label of ['Memory used', 'Check time', 'Total time']) {
  console.log(
    `  ${label}: ${parseInformativeDiagnostic(compilerOutput, label)} (informative)`
  )
}
