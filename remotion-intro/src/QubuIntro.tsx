...
const ParameterScene = () => {
  ...
  return (
    <Shell dark>
      <Pop delay={3}><Eyebrow>03 · Values become parameters</Eyebrow></Pop>
      <Pop delay={9}><BigText style={{fontSize: portrait ? 82 : 92, marginTop: 28}}>The value stays<br /><span style={{color: colors.mint}}>out of the SQL.</span></BigText></Pop>
      <div style={{display: 'grid', gridTemplateColumns: portrait ? '1fr' : '0.8fr 1.2fr', gap: portrait ? 34 : 70, alignItems: 'center', marginTop: portrait ? 100 : 78}}>
        <Pop delay={20}>
          <div style={{border: '2px solid #373B39', borderRadius: 26, padding: portrait ? 40 : 36, fontFamily: mono, fontSize: portrait ? 30 : 28, background: '#202321'}}>
            where(eq(users.id, <span style={{color: colors.mint}}>42</span>))
          </div>
        </Pop>
        <Pop delay={34}>
          <div style={{borderRadius: 26, padding: portrait ? 40 : 36, background: colors.white, color: colors.ink, boxShadow: shadow}}>
            <div style={{fontFamily: mono, fontSize: portrait ? 27 : 27, lineHeight: 1.55}}>WHERE (&quot;users&quot;.&quot;id&quot; = ?)</div>
            <div style={{fontFamily: mono, fontSize: portrait ? 27 : 24, color: colors.blue, marginTop: 22}}>parameters: [42]</div>
          </div>
        </Pop>
      </div>
    </Shell>
...
const DialectScene = () => {
  ...
  return (
    <Shell accent>
      <Pop delay={2}><Eyebrow><span style={{color: colors.mint}}>04 · Render explicitly</span></Eyebrow></Pop>
...
const ViteScene = () => {
  ...
  return (
    <Shell>
      <Pop delay={3}><Eyebrow>05 · Optional Vite plugin</Eyebrow></Pop>
      <Pop delay={9}><BigText style={{fontSize: portrait ? 84 : 94, marginTop: 28}}>Skip the import wall.</BigText></Pop>
      <div style={{display: 'grid', gridTemplateColumns: portrait ? '1fr' : '1fr 80px 1fr', alignItems: 'center', gap: portrait ? 28 : 24, marginTop: portrait ? 110 : 90}}>
        <Pop delay={18}>
          <div style={{borderRadius: 26, padding: portrait ? 42 : 38, background: colors.white, border: '2px solid rgba(21,23,22,.09)', boxShadow: shadow}}>
            <div style={{fontSize: 20, color: colors.muted, fontWeight: 700, letterSpacing: '.13em', textTransform: 'uppercase'}}>Without the plugin</div>
            <div style={{fontFamily: mono, fontSize: portrait ? 26 : 24, lineHeight: 1.5, marginTop: 28, color: colors.muted}}>
              import {'{'} eq, from, integer,<br />select, table, text, where {'}'}<br />from <span style={{color: colors.coral}}>'qubu'</span>
            </div>
          </div>
        </Pop>
        <Pop delay={30} style={{fontSize: 50, color: colors.blue, textAlign: 'center', transform: portrait ? 'rotate(90deg)' : undefined}}>→</Pop>
        <Pop delay={38}>
          <div style={{borderRadius: 26, padding: portrait ? 42 : 38, background: colors.ink, color: colors.white, boxShadow: shadow}}>
            <div style={{fontSize: 20, color: colors.mint, fontWeight: 700, letterSpacing: '.13em', textTransform: 'uppercase'}}>With the plugin</div>
            <div style={{fontFamily: mono, fontSize: portrait ? 32 : 31, marginTop: 34}}><span style={{color: colors.coral}}>'use qubu'</span></div>
          </div>
        </Pop>
      </div>
      <Pop delay={54} style={{marginTop: portrait ? 82 : 60}}>
        <div style={{color: colors.muted, fontSize: portrait ? 30 : 27}}>It inserts only the Qubu imports that file uses.</div>
      </Pop>
    </Shell>
  );
};
    ...
    <Shell>
      <div style={{height: '100%', display: 'grid', placeItems: 'center', textAlign: 'center'}}>
        <div>
          ...
          <Pop delay={12}><BigText style={{fontSize: portrait ? 100 : 118, marginTop: 50}}>Qubu builds SQL.<br /><span style={{color: colors.blue}}>Your driver runs it.</span></BigText></Pop>
          <Pop delay={20}><div style={{fontSize: portrait ? 28 : 26, color: colors.muted, marginTop: 34}}>Typed composition. Explicit execution.</div></Pop>
          <Pop delay={26}>
            <div style={{marginTop: 44, display: 'inline-flex', alignItems: 'center', gap: 18, borderRadius: 100, padding: '20px 32px', background: colors.ink, color: colors.white, fontFamily: mono, fontSize: portrait ? 27 : 25}}>
...
export const QubuIntro = () => (
  <AbsoluteFill style={{fontFamily}}>
    <Sequence durationInFrames={100}><Opening /></Sequence>
    <Sequence from={100} durationInFrames={170}><QueryScene /></Sequence>
    <Sequence from={270} durationInFrames={140}><TypeScene /></Sequence>
    <Sequence from={410} durationInFrames={120}><ParameterScene /></Sequence>
    <Sequence from={530} durationInFrames={120}><DialectScene /></Sequence>
    <Sequence from={650} durationInFrames={150}><ViteScene /></Sequence>
    <Sequence from={800} durationInFrames={100}><Finale /></Sequence>
  </AbsoluteFill>
);
