...
> Use this matrix to decide which SQLite schema facts Qubu v2 can serialize and which combinations must be diagnosed before a snapshot is written.
...
| Schema fact         | SQLite v2 behavior                                                                                                                                                                                                                            |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Foreign keys        | Standard update/delete actions and `MATCH SIMPLE` are retained. `MATCH FULL` and `MATCH PARTIAL` are diagnosed because the v2 adapter does not claim SQLite support for them.                                                                 |
