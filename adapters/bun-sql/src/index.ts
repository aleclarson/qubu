...
export interface BunSqlResult<TRow extends object = Record<string, unknown>>
  extends Array<TRow> {
  readonly count?: number
}

export interface BunSqlExecutor {
  unsafe<TRows extends object[]>(
    text: string,
...
export interface BunSqlTransactionAdapter
    ...
    transaction(callback, transactionOptions: TransactionOptions = {}) {
      ...
      return sql.begin(async transaction => {
        const result = await callback(
          executionAdapter(transaction, dialect, encoder)
        )
        throwIfAborted(transactionOptions.signal)
        return result
      })
  ...
  return {
    ...
    async execute<TRow extends object>(request: ExecutionRequest) {
      ...
      return {
        ...
        ...(isMutation && result.count !== undefined
