      ...
      const statement = database
        ...
        .bind(
          ...request.statement.parameters.map((value, index) =>
            encodeD1Parameter(value, options.encoder, request.statement.parameterSqlTypes?.[index]),
          ),
        )
...
function encodeD1Parameter(
  value: unknown,
  encoder: DriverValueEncoder | undefined,
  ...
  const encoded = encoder === undefined ? value : encoder.encode(value, sqlType)
