  ...
  const plugin = {
    ...
    schema: {
      team: {
        ...
        indexes: [{ name: 'team_owner_slug', fields: ['ownerId', 'slug'] }],
      },
    },
  ...
  const result = betterAuthSchema(
    {
      ...
      user: {
        additionalFields: {
          sequence: { type: 'number', bigint: true },
        },
      },
    },
    'sqlite'
  ...
  const result = betterAuthSchema(
    {
      ...
      user: {
        additionalFields: {
          activatedAt: { type: 'date', defaultValue: () => new Date() },
        },
      },
    },
    'postgresql'
    ...
    betterAuthSchema(
      {
        user: {
          additionalFields: {
            managerId: {
              type: 'string',
              references: { model: 'user', field: 'missing' },
            },
          },
        },
      },
