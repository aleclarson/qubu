...
test("maps configured physical field names in single-row updates", async () => {
  ...
  expect(fake.requests[0]?.statement.text).toContain('UPDATE "user" SET "full_name" = ?')
})
...
test.each([
...
] as const)("keeps %s single-row mutations atomic and limited", async (_name, dialect) => {
  ...
  expect(fake.requests[0]?.statement.text).toContain('WHERE (("user"."name" = ')
...
test("uses a locked transaction for MySQL single-row consumption", async () => {
  ...
  expect(fake.requests[1]?.statement.text).toContain("DELETE FROM `user` WHERE (`user`.`id` = ?)")
})
...
test("keeps MySQL Date values in driver parameters", async () => {
  ...
  expect(fake.requests[0]?.statement.parameters).toEqual(expect.arrayContaining([now]))
})
...
test("rejects unsupported dialects and non-transactional clients", () => {
  ...
  expect(() => qubuAdapter(qubu(unsupported))).toThrow(/supports PostgreSQL, MySQL, and SQLite/)
})
